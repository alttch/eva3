    ...
    try:
        ...
        for ks in cfg.sections():
            try:
                ...
                try: key.sysfunc = (cfg.get(ks, 'sysfunc') == 'yes')
                except: pass
                try:
                ...
                if _ha:
                    try:
                        _hosts_assign = list(filter(None,
                            [x.strip() for x in _ha.split(',')]))
                        key.hosts_assign = \
                        ...
                        eva.core.log_traceback()
                try:
                    key.item_ids = list(filter(None,
                        [x.strip() for x in cfg.get(ks, 'items').split(',')]))
                except: pass
                try: key.groups = list(filter(None,
                ...
                try: key.allow = list(filter(None,
...
def check(k, item = None, allow = [], pvt_file = None, ip = None,
        master = False, sysfunc = False):

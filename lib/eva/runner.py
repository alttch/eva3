...
class ExternalProcess(object):
    ...
    def __init__(self, fname, item = None, env = None, update = False,
            args = None, timeout = None, tki = None):
        if item:
            if item.virtual:
    ...
    def set_tki(self, tki):
        ...
        else: tki = eva.core.timeout - default_tki_diff
        if tki < 0: tki = 0
    ...
    def launch(self):
        try:
            self.term = threading.Thread(target = self._t_term,
                    name = 'runner_t_term_%f' % time.time())
    ...
    def _t_term(self):
        ...
        try: pp = psutil.Process(self.xc.pid)
        except: return
        childs = []
        try:
            for child in pp.children(recursive=True): childs.append(child)
        except: pass
        try: self.xc.terminate()
        except: pass
        for child in childs:
            try: child.terminate()
            except: pass

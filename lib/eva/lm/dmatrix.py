...
class DecisionMatrix:
    ...
    def exec_rule_action(self, event_code, rule, item):
        ...
        if rule.macro:
            if rule.block_on_exec:
                self.run_macro(event_code, rule, item)
            else:
...
class DecisionRule(eva.item.Item):
    ...
    def is_mask(self):
        return not self.for_item_type or \
                self.for_item_type == '#' or \
                not self.for_item_group or \
    ...
    def get_item_oid(self):
        ...
        ) else f'{self.for_item_type}:{self.for_item_group}/{self.for_item_id}'
    ...
    def serialize(self,
        ...
        d['block_on_exec'] = self.block_on_exec
    ...
    def update_config(self, data):
        ...
        if 'block_on_exec' in data:
            self.block_on_exec = data['block_on_exec']
        if 'priority' in data:

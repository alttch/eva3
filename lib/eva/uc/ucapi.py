...
class UC_API(GenericAPI):
    ...
    def state(self, k = None, i = None, full = False,
            group = None, tp = None):
        if i:
    ...
    def update(self, k = None, i = None, status = None, value = None,
            force_virtual = 0):
        item = eva.uc.controller.get_item(i)
        if not item or not apikey.check(k, item): return False
        return item.update_set_state(status = status, value = value,
                force_virtual = force_virtual)



    def action(self, k = None, i = None, action_uuid = None, nstatus = None,
            nvalue = '', priority = None,
            q_timeout = None, wait = 0):
    ...
    def result(self, k = None, uuid = None, item_id = None,
            group = None, state = None):
        if uuid:
            ...
            if item_id:
                ...
                if item_id.find('/') > -1:
                    if item_id in eva.uc.controller.Q.actions_by_item_full_id:
                      ar = eva.uc.controller.Q.actions_by_item_full_id[item_id]
                else:
            ...
            for a in ar:
                ...
                if (state == 'Q' or state =='queued') and \
                        not a.is_status_queued(): continue
                elif (state == 'R' or state == 'running') and \
                        not a.is_status_running(): continue
                elif (state == 'F' or state == 'finished') and \
                        not a.is_finished(): continue
                result.append(a)
            return result
...
# master functions for item configuration
    ...
    def create_unit(self, k = None, unit_id = None, group = None,
            virtual = False, save = False):
        if not apikey.check(k, master = True): return None
        return eva.uc.controller.create_unit(unit_id = unit_id, 
                group = group, virtual = virtual, save = save)


    def create_sensor(self, k = None, sensor_id = None, group = None,
            virtual = False, save = False):
        if not apikey.check(k, master = True): return None
        return eva.uc.controller.create_sensor(sensor_id = sensor_id, 
                group = group, virtual = virtual, save = save)


    def create_mu(self, k = None, mu_id = None, group = None,
            virtual = False, save = False):
        if not apikey.check(k, master = True): return None
        return eva.uc.controller.create_mu(mu_id = mu_id, 
                group = group, virtual = virtual, save = save)
...
class UC_HTTP_API(GenericHTTP_API, UC_API):
    ...
    def state(self, k = None, i = None, full = None,
            g = None, p = None):
        if full:
            _full = True
        else:
    ...
    def update(self, k = None, i = None, s = None, v = None,
            force_virtual = ''):
        if force_virtual:
            _fv = True
        else:
    ...
    def action(self, k = None, i = None, u = None,
            s = None, v = '', p = None, q = None, w = 0):
        try:
            _s = int(s)
        except:
            raise cp_api_error('status is not an integer')
    ...
    def create_unit(self, k = None, i = None, g = None,
            virtual = None, save = None):
        cp_need_master(k)
        return http_api_result_ok() if super().create_unit(
                k, i, g, virtual, save) else http_api_result_error()


    def create_sensor(self, k = None, i = None, g = None,
            virtual = None, save = None):
        cp_need_master(k)
    ...
    def create_mu(self, k = None, i = None, g = None,
            virtual = None, save = None):
        cp_need_master(k)
...
def start():
    ...
    cherrypy.tree.mount(UC_HTTP_Root(), '/', config = {
            '/favicon.ico': {
    ...
    cherrypy.tree.mount(object(),'/uc-ei',config =
        {'/': {

...
class Unit(eva.item.UpdatableItem, eva.item.ActiveItem):
    ...
    def create_action(self, nstatus, nvalue = 'null', priority = None,
            action_uuid = None):
        ...
        while self.auto_processor_active and self.auto_off:
            ...
            if self.last_action and \
                    ...
                    time.time() - self.last_action > self.auto_off:
                        ...
                        eva.uc.controller.exec_unit_action(self, 0, None,
                                wait = eva.core.timeout)
        self.auto_processor_active = False
    ...
    def update_set_state(self, status = None, value = None,
            from_mqtt = False, force_virtual = 0):
        ...
        self.set_state(status = _status, value = value,
                nstatus = nstatus, nvalue = nvalue, from_mqtt = from_mqtt)
        self.queue_lock.release()
        return True


    def set_state(self, status = None, value = None,
            nstatus = None,nvalue = None, from_mqtt = False):
        if self._destroyed: return False
        need_notify = False
...
class UnitAction(eva.item.ItemAction):
    ...
    def __init__(self, unit, nstatus, nvalue = None, priority = None,
            action_uuid = None):
    ...
    def set_status(self, status, exitcode = None,
        ...
        result = super().set_status(status = status,
                exitcode = exitcode, out = out, err = err, lock = lock)
        if not result:
        ...
        if self.is_status_running():
            ...
            if self.item.update_exec_after_action:
                ...
                smsg = 'status=%u value="%s"' % (self.item.status,
                        self.item.value)
            logging.debug('action %s completed, %s %s' % (self.uuid,
                self.item.full_id, smsg))
        if lock: self.unit_action_lock.release()
        return True
    ...
    def action_env(self):
        ...
        e = {
            'EVA_NSTATUS': str(self.nstatus),
            'EVA_NVALUE': str(nvalue)
            }

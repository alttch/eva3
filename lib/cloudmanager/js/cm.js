...
manage_controllers['status_labels']['job']['periods'] = ['second', 'seconds', 
  'minute', 'minutes', 'hour', 'hours', 'day', 'days', 'week', 'weeks', 
  'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']
manage_controllers['locked_controller'] = [];
manage_controllers['items']['module_groups'] = ['phi', 'lpi'];
  ...
  if(!uc_block.find('.mng_btn_holder')[0]) {
    ...
    var mngBtns = [{
      ...
      action: function() {
        ...
        $call(id, 'list_' + cur_group + '_mods', null, function(data) {
          insertModulesList(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          }
  ...
  $.each(result.data, function(k, v) {
    ...
    if(manage_controllers['reload_units'] && manage_controllers['items'][v.oid] && 
      ...
      $('<button />', {
        ...
        onclick: 'get_unit_props_for_state("' + 
          manage_controllers["current_controller"] + 
          '", "' + v.oid + '")',
        html: 'SET'
      }).appendTo(unit_btns);
...
function insertDriversList(result) {
  if(result.code === 0) {
    var ih = $('#mng_' + manage_controllers['current_item']);
    var ig;
    var cur_group;
    if($('#drivers_holder')[0]) {
      ...
      //   class: 'ctrl_reload_btn'
      // }).click(function() {
      //   get_driver_for_edit(manage_controllers['current_controller']);
      // }).html('Create driver').appendTo(ch);
      var cur_group = $('<div />', {
        class: 'group_holder',
        id: 'driver_items'
      }).appendTo(ih);
    ...
    $.each(result.data, function(k, v) {
      ...
      $('<td />').html(v.mod).appendTo(drv_holder);
      $('<td />').html(v.lpi_id).appendTo(drv_holder);
      $('<td />').html(v.phi_id).appendTo(drv_holder);
      drv_holder.appendTo(tbody);
    });
    drv_table.append(tbody);
    cur_group.append(drv_table);
    if(tbody.html() == '') {
      $('<div />', {
...
function insertModulesList(result) {
  if(result.code === 0 && result.data) {
    ...
    if($('#mng_' + manage_controllers['current_item'] + '_group')[0]) {
      ...
      ig = $('<select />', {
        ...
        hideManageGroupHolders(ih);
        showCurrentGroupHolder(this.value);
        $call(manage_controllers["current_controller"], 
          'list_' + this.value + '_mods', null, insertModulesList);
      });
      ...
      $('<button />', {
        ...
        class: 'ctrl_reload_btn',
        onclick: 'module_load_dialog(manage_controllers["current_controller"])',
      }).css({display: 
        manage_controllers['items']['current_module_groups'] == 'lpi' ? 
          'none' : 'inline-block'
      }).html('Load module')
      .appendTo(ch);
      $.each(manage_controllers['items']['module_groups'], function(k, v) {
    ...
    if (manage_controllers['items']['current_module_groups'] == 'phi') {
    ...
    $.each(result.data, function(k, v) {
      ...
      $('<td />').html(v.mod).appendTo(mod_holder);
      $('<td />').html(v.version).appendTo(mod_holder);
      $('<td />').html(v.description).appendTo(mod_holder);
      if (manage_controllers['items']['current_module_groups'] == 'phi') {
        $('<td />').html(v.equipment).appendTo(mod_holder);
      }
      $('<td />').html(v.api).appendTo(mod_holder);
      if (manage_controllers['items']['current_module_groups'] == 'phi') {
        var unlink_btn = $('<td />').html('').appendTo(mod_holder);
        if (v.id) {
          $('<button />', {
...
function get_unit_props_for_state(id, oid) {
  $call(id, 'list_props', { i: oid }, function(res) {
    var data = res.data;
    if(data && res.code === 0) {
      unit_state_dialog(id, oid, data);
    } else {
      $popup('error', 'ERROR', '<div class="content_msg">' +
...
function unit_state_dialog(id, oid, data) {
  ...
  $.each(data.status_labels, function(_k, v) {
...
function module_load_dialog(id) {
  var html =
    '<form class="form-horizontal">' +
    // line 1
    '<div class="form-group">' +
    ...
    '<input class="form-control" type="text" size="5"' +
    'id="phi_config" value="" /></div></form>';
  $popup('confirm', 'Link module',
    html, {
      btn1: 'OK',
      btn2: 'CANCEL',
      btn1a: function() {
...
function edit_macro_dialog(id, oid, props) {
  ...
  html += '/>' +
    ...
    'data-on="ON"></label></div></div></div></form>';
...
function validate_job_dialog() {
  ...
  if(job.every) {
    var singlePeriod = ['second', 'seconds', 'minute', 'minutes', 'hour', 'hours',
    ...
    var periodWithFullTime = ['day', 'days', 'week', 'weeks', 'monday', 
                              'tuesday', 'wednesday', 'thursday', 'friday', 
                              'saturday', 'sunday'];
    var periodWithPartTime = ['hour', 'hours'];
    ...
    if(time) {
      ...
      if (timeParts.length < 2 || timeParts.length > 3 ||
            periodWithPartTime.includes(period) && timeParts.length == 3) {
        $('#job_every_time').addClass('device-error');
        return false;
      }
      for(var i in timeParts) {
        if (timeParts[i].length != 2 || !isInt(timeParts[i]) || 
              Number(timeParts[i]) < 0 || Number(timeParts[i]) > 59) {
          $('#job_every_time').addClass('device-error');
          return false;
        }
      }
      if(periodWithFullTime.includes(period) && Number(timeParts[0]) > 23) {
        $('#job_every_time').addClass('device-error');
        return false;
      }
  ...
  if(kwargs) {
    ...
    for(var i=0; i<kwargs_arr.length; i++) {
      var values = kwargs_arr[i].trim().split('=');
      if(!kwargs_arr[i].includes('=') || 
          values.length != 2 || !(values[0] && values[1]) ||
            isNumeric(values[0]) && isNumeric(values[1])) {

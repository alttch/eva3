...
function update_remote_units(data) {
  ...
  $.each(data, function(k, v) {
    var datarow = $('<tr />', {
      class: 'clickable_row',
      onclick: 'unit_state_dialog("' + v.controller_id + '", "' + v.oid + '")',
    });
...
function update_remote_lvars(data) {
  ...
  $.each(data, function(k, v) {
    var datarow = $('<tr />', {
      class: 'clickable_row',
      onclick: 'lvar_state_dialog("' + v.controller_id + '", "' + v.oid + '")',
    });
...
function update_remote_macros(data) {
  ...
  $.each(data, function(k, v) {
    var datarow = $('<tr />', {
      class: 'clickable_row',
      onclick: 'prepare_macro_run("' + v.controller_id + '", "' + v.oid + '")',
    });
    datarow.append($('<td />').html(v.controller_id));
    datarow.append(
      $('<td />').html(
  ...
  if(!uc_block.find('.mng_btn_holder')[0]) {
    ...
    var mngBtns = [{
      ...
      action: function() {
        ...
        var api_action = 'list_' + cur_group + (cur_group == 'lpi' ? '_mods' : '');
    ...
    if($('#mng_' + manage_controllers['current_item'] + '_group')[0]) {
      ...
      ig = $('<select />', {
        ...
        $call(manage_controllers["current_controller"], 
          'list_' + this.value + '_mods', null, function(res) {
             insertModulesList(res, 'lpi')
          });
        } else if(this.value == 'phi') {
          $call(manage_controllers["current_controller"], 
            'list_' + this.value, null, function(res) {
              insertModulesList(res, 'phi')
          });
        }
      });
      ...
      $.each(result.data, function(k, v) {
        ...
        if(manage_controllers['reload_phi'] && 
          ...
          if($('#'+$.escapeSelector('phi_' + v.id))[0]) {
            $('#'+$.escapeSelector('phi_' + v.id)).html(phi.html());
          } else {
            $('#modules_phi').append(phi);
          }
        }
      });
...
function edit_macro_dialog(id, oid, props) {
  ...
  html += '<div id="macro_editor"></div></form>' +

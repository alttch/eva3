...
function get_phi_ports(controller, id) {
  $call(controller, 'get_phi_ports', { i: id }, function(res) {
    var data = res.data;
    var msg = '<table><tr><th>Port</th><th>Name</th><th>Description</th></tr>';
    var popt = 'success';
    if(data && res.code === 0) {
      for(var i in data) {
        msg += '<tr><td>' + data[i].port + '</td><td>' + data[i].name + '</td><td>' + data[i].description + '</td><td></tr>';
      }
      msg += '</table>';
    } else {
      popt = 'error';
      msg = 'Server error. Unable to get module ports. Result: ' + data.result;
    }
    VanillaToasts.create({
      type: popt,
      text: msg,

...
function update_sensor_state(id, oid) {
  ...
  $call(id, 'update', {
      ...
      if (data && data.ok) {
        ...
        msg += 'Sensor state not changed. Result: ' + res.result + '</div>';
        $popup('error', 'ERROR', msg);
      }
    }, function() {
      $popup('error', 'ERROR', '<div class="content_msg">Server error</div>');
...
function edit_macro_src(id,oid) {
  ...
  $call(id, 'set_macro_prop', {
      ...
      var msg = '<div class="content_msg">';
      if(data && data.ok) {
        msg += 'Result: src changed successfully</div>';
        $popup('info', 'SUCCESS', msg);
      } else {
        msg += 'Unable set props. Result: ' + data.result + '</div>';
        $popup('error', 'ERROR', msg);
    ...
    $popup('error', 'ERROR', '<div class="content_msg">' +
      'Server error. Unable to set macro src</div>');
  });
}
...
function phi_for_load_dialog(controller) {
  $call(controller, 'list_phi', null, function(res) {
    ...
    if(data && res.code === 0) {
      ...
      $popup('error', 'ERROR', '<div class="content_msg">' +
        'Unable get phi list. Result: ' + data.result + '</div>');
    }
  }, function() {
    $popup('error', 'ERROR', '<div class="content_msg">' +
      'Server error. Unable to get phi list</div>');
  });
}
...
function get_macro_src_for_edit(id, oid) {
  $call(id, 'get_macro', { i: oid }, function(res) {
    ...
    if(data && res.code === 0) {
      ...
      $popup('error', 'ERROR', '<div class="content_msg">' +
        'Unable get src. Result: ' + data.result + '</div>');
    }
  }, function() {
    $popup('error', 'ERROR', '<div class="content_msg">' +
      'Server error. Unable to get macro src</div>');
  });
}

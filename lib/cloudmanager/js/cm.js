...
function update_remote(data) {
  var cols = ['Controller', 'Type', 'ID', 'S', 'Val'];
  $('#tbl-remote-items')
    .find('tr')
    .remove()
    .end();
  var header = $('<tr />', {class: 'tbl-remote-items-hdr'});
  $.each(cols, function(k, v) {
    header.append($('<td />').html(v));
  });
  $('#tbl-remote-items').append(header);
  $.each(data, function(k, v) {
    ...
    datarow.append(
      $('<td />')
        .addClass('vtop')
        .html(
          v.full_id + (v.description?'<br /><span class="item-descr">(' +
            v.description +
            ')</span>':'')
        )
    );
    ...
    $('#tbl-remote-items').append(datarow);
  });
}

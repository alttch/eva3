...
function append_controller(c, ct) {
  ...
  if (c.managed && c.connected) {
    ...
    cel.click(function() {
      $popup('warning', 'Rights failre', '<div class="content_msg">' +
        'You don\'t have permissions for managing the controller</div>');
    });
    cel.addClass('controller-connected');
  } else {
    cel.click(function() {
      $popup('warning', 'Connection failre', '<div class="content_msg">' +
        'First you have to connect controller</div>');
    });
...
function remove_unit(id, oid) {
  $call(id, 'destroy', {i: oid}, function(res) {
    ...
    if (data && data.ok) {
      ...
      var msg = '<div class="content_msg">'+ 'Kill command failed for ' + 

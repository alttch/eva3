...
function insertUnitsList(result) {
  ...
  $.each(result.data, function(k, v) {
    ...
    if(manage_controllers['reload_units'] && manage_controllers['items'][v.oid] && 
      ...
      if($('#'+eoid)[0]) {
        $('#'+eoid).html(unit.html());
        $('[id^=radio_mng_' + eoid + '_][value=' + v.status + ']')
          .attr('checked', true);
      } else {
...
function redraw_unit(v) {
  ...
  $('[id^=radio_mng_' + eoid + '_]').removeAttr('checked');
  $('[id^=radio_mng_' + eoid + '_][value=' + v.status + ']').attr('checked', true);
  var eb = $('[id="btn_enable_' + eoid + '"]');
  if (eva_sfa_states[v.oid].action_enabled) {
    $('[id^=radio_mng_' + eoid + '_]').removeAttr('disabled');
    eb.html('ENABLED');
    ...
    $('[id^=radio_mng_' + eoid + '_]').prop('disabled', 'disabled');
  }
}

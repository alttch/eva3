...
function load_driver(id) {
  ...
  params.c = $('#driver_config').val().trim();
...
function driver_load_dialog(controller, id, data) {
  ...
  $popup('confirm', id ? 'Set Driver ' + id : 'Load driver',
    html, {
      btn1: 'OK',
      btn2: 'CANCEL',
      btn1a: function() {
      ...
      va: function() {
        if(!id) {
          return validate_load_driver_dialog();
        } else {
          return true
        }
      }
...
function validate_load_driver_dialog() {
  var _v = true;
  $('#driver_lpi').removeClass('device-error');
  $('#driver_phi').removeClass('device-error');

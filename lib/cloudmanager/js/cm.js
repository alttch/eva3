  ...
  $.each(result.data, function(k, v) {
    ...
    if(manage_controllers['reload_units'] && manage_controllers['items'][v.oid] && 
      ...
      $('<button />', {
        ...
        onclick: 'drivers_list_for_item_assign("' + 
          manage_controllers["current_controller"] + 
          '", "' + v.oid + '")',
        html: 'ASSIGN'
      }).appendTo(unit_btns);
...
function drivers_list_for_item_assign(id, oid) {
  $call(id, 'list_drivers', null, function(res) {
    var data = res.data;
    if(data && res.code === 0) {
      unit_assign_dialog(id, oid, data);
    } else {
      VanillaToasts.create({
        type: 'error',
...
function unit_assign_dialog(id, oid, data) {
  ...
  html += '</select></div>' + 
    ...
    '<input class="form-control" type="text" size="5"' +
    'id="item_configuration" value="" /></div></form>';
...
function get_sensor_props_for_edit(id, oid) {
  $call(id, 'list_props', { i: oid }, function(res) {
    var data = res.data;
    if(data && res.code === 0) {

...
function manage_uc(controller_id) {
  $call(controller_id, 'test', null, show_debug_info);
}
...
function edit_controller_props(controller_id) {
  eva_sfa_call(
    ...
    function(data) {
      ...
      form.append($('<label />').html('Use local masterkey'));
      var cb = $('<input />', {
...
function showCurrentController(type, id) {
  if(type == -1) {
    ...
    $('#ctrl-' + $.escapeSelector(id)).show();
    if(type == 'lm') {
      show_lm_block(type, id);
    } else if(type == 'uc') {
      show_uc_block(type, id);
    }
...
function show_lm_block(type, id) {
  ...
  if(!lm_block.find('.mng_btn_holder')[0]) {
    ...
    manage_lm(id);
    setInterval(function() {manage_lm(id)}, 10000);
...
function show_uc_block(type, id) {
  ...
  if(!uc_block.find('.mng_btn_holder')[0]) {
    ...
    var mngBtns = [{
      ...
      action: function() {
        ...
        manage_controllers['current_item'] = 'sensors';
          manage_controllers['current_item'] = 'sensors';
          $call(id, 'groups', {p: 'S'}, insertGroups, $log);
          manage_controllers['current_interval'] = setInterval(function() {
    ...
    manage_uc(id);
    setInterval(function() {manage_uc(id)}, 10000);
    $.each(mngBtns, function(k, v) {
      insertMngButton(btn_holder, v, k);
      $('<div />', {
        id: 'mng_' + v.type,
    ...
    manage_controllers['current_item'] = $('.ctrl_content .mng_items_holder:visible')[0].id.substr(4);
  }
}
...
function insertGroups(result) {
  ...
  if($('#mng_' + manage_controllers['current_item'] + '_group')[0]) {
    ...
    if(f) {
      ...
      .html('Create ' + manage_controllers['current_item'])
      .appendTo(ch);
    }
  }
  loadItemsList();
...
function insertUnitsList(result) {
  ...
  $.each(result.data, function(k, v) {
    ...
    if(manage_controllers['reload_units'] && manage_controllers['items'][v.oid] && 
      ...
      .appendTo(unit_btns);
      $('<label />', {for: 'btn_' + v.oid}).appendTo(unit_btns);    
      if(v.action_enabled) {
        $('<button />', {
  ...
  if(result.code === 0) {
    ...
    if($('#mng_' + manage_controllers['current_item'] + '_group')[0]) {
      ...
      $('<button />', {
        ...
        edit_rule_dialog(manage_controllers['current_controller']);
      }).html('New rule').appendTo(ch);
  ...
  $call(id, s+'_actions', {i: oid}, function(res) {
    ...
    if(data && data.ok) {
      manage_controllers['items'][oid].action_enabled = 
        s == 'enable' ? true : false;
      manage_controllers["current_controller"] = id;
      redraw_unit(manage_controllers['items'][oid]);
    } else {
...
function enable_disable_sensor(id, oid, s) {
  $call(id, 'update', {
      ...
      if(data && data.ok) {
        ...
        redraw_enable_sensor_btn(manage_controllers['items'][oid]);
      } else {
        $popup('error', 'ERROR', '<div class="content_msg">' +
...
function update_unit_state(id, oid) {
  ...
  $call(id, 'update', {
      ...
      if (res.data && res.data.ok) {
        ...
        $popup('error', 'ERROR', '<div class="content_msg">' +
          'Unit state not changed. Result: ' + res.result + '</div>'
        );
      }
    }, 
    function() {
...
function create_lvar() {
  ...
  var id = manage_controllers['current_controller'];
  $call(id, 'create_lvar', {i: name, g: group}, function(res) {
    var data = res.data;
    var msg = '<div class="content_msg">';
    if (data && data.id) {
      msg += 'Lvar created';
...
function set_lvar_state(id, oid) {
  ...
  $call(id, 'set', {i: oid, s: s, v: v}, 
    function(res) {
      ...
      if (data && data.ok) {
        ...
        $popup('error', 'ERROR', '<div class="content_msg">' +
          'LVar not changed. </br>Result: ' + data.error + '</div>');
      }
    }, function() {
      $popup('error', 'ERROR', '<div class="content_msg">' +
  ...
  $call(id, 'set_macro_prop', { i: oid, p: '', v: {
    ...
    if(data && data.ok) {
      $popup('info', 'SUCCESS', '<div class="content_msg">' +
        'Result: props set successfully</div>');
    } else {
      $popup('error', 'ERROR', '<div class="content_msg">' +
        'Unable set props. Result: ' + data.result + '</div>');
    }
  }, function() {
    $popup('error', 'ERROR', '<div class="content_msg">' +
...
function create_cycle() {
  ...
  var id = manage_controllers['current_controller'];
  $call(id, 'create_cycle', {i: name, g: group}, function(res) {
    var data = res.data;
    var msg = '<div class="content_msg">';
    if (data && data.id) {
      msg += 'Cycle created';
...
function select_unit_state(id, oid) {
  var html =
    '<form class="form-horizontal" id="set_unit_state">' +
...
function select_lvar_state(id, oid) {
  var html = 
    '<form class="form-horizontal" id="set_lvar_state">' +
    '<div class="form-group">' +
    '<label>Status</label>' +
  ...
  var value = manage_controllers['items'][oid].value;
  html +=
    '</div></div></div>' +
    '<div class="form-group">' +
...
function edit_rule_dialog(id, i) {
  ...
  html +=
    ...
    'id="rule_description" oninput="rule_descr_size(event)" ' +
    'maxlength="50">' + _description + '</textarea>' +
    '<div class="used_symbols">Used <span id="descr_size">0</span> ' +
    'symbols from 50</div></div>';
  // row 3
  html +=
    '<div class="form-group">' +
  ...
  html +=
    '<div class="col-12 col-sm-4 row-schf">' +
    '<label for="rule_break">Break</label>' +
  ...
  html +=
    ...
    '<input class="form-control" type="text" size="5"' +
    'id="rule_macro" value="' + _macro + '" /></div>';
  html +=
    '<div class="col-12 col-sm-4 row-schf">' +
  ...
  html +=
    ...
    '<input class="form-control" type="text" size="5"' +
    'oninput="rule_descr_size(event)"' +
    'id="rule_macro_kwargs" value="' + _macro_kwargs + '" />' +
    '<div class="used_symbols">Used <span id="kwargs_size">0</span> ' +
    'symbols from 50</div></div></div>';
  // end form
  html += '</form>';
  ...
  $('#descr_size').html($('#rule_description').val().length);
  $('#kwargs_size').html($('#rule_macro_kwargs').val().length);
...
function create_unit_dialog() {
  var html =
    ...
    '<input class="form-control" type="text" size="5"' +
    'id="unit_name" value="" /></div></form>';
  $popup('confirm', 'Create unit',
    html, {
      btn1: 'CREATE',
      btn2: 'CANCEL',
      btn1a: function() {
...
function create_lvar_dialog() {
  var html =
    ...
    '<input class="form-control" type="text" size="5"' +
    'id="lvar_name" value="" /></div></form>';
  $popup('confirm', 'Create lvar',
    html, {
      btn1: 'CREATE',
      btn2: 'CANCEL',
      btn1a: function() {
...
function create_macro_dialog() {
  var html =
    ...
    '<input class="form-control" type="text" size="5"' +
    'id="macro_name" value="" /></div></form>';
  $popup('confirm', 'Create macro',
    html, {
      btn1: 'CREATE',
      btn2: 'CANCEL',
      btn1a: function() {
...
function edit_macro_dialog(id, oid, props) {
  var html =
    ...
    'id="action_enabled" ' + (props.action_enabled?'checked':'') + '/>' +
    ...
    'id="action_exec" value="' + props.action_exec + '"/></div>' +
    // line 3
    '<div class="form-group">' +
    '<label class="control-label">Pass errors:</label>' +
    '<div class="custom_chbox_holder">' +
    '<input class="custom_chbox" type="checkbox" ' +
    'id="pass_errors" ' + (props.pass_errors?'checked':'') + '/>' +
    ...
    '<div class="custom_chbox_holder">' +
    '<input class="custom_chbox" type="checkbox" ' +
    'id="send_critical" ' + (props.send_critical?'checked':'') + '/>' +
    '<label for="send_critical"></label></div></div></form>';
  $popup('confirm', 'Create cycle',
    html, {
      btn1: 'EDIT',
      btn2: 'CANCEL',
      btn1a: function() {
...
function edit_cycle_dialog(id, oid, props) {
  var html =
    ...
    '<div class="custom_chbox_holder">' +
    '<input class="custom_chbox" type="checkbox" ' +
    'id="cycle_autostart" ' + (props.autostart?'checked':'') + '/>' +
...
manage_controllers['create_units'] = create_unit_dialog;
manage_controllers['create_sensors'] = create_sensor_dialog;
manage_controllers['create_lvars'] = create_lvar_dialog;
manage_controllers['create_macros'] = create_macro_dialog;
manage_controllers['create_cycles'] = create_cycle_dialog;
...
function rule_descr_size(e) {
  var target = e.currentTarget;
  $(target).next('.used_symbols').find('span').html(target.value.length);
}

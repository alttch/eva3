  ...
  $call(id, 'set_prop', {
      ...
      if(data && data.ok) {
        $popup('info', 'SUCCESS', '<div class="content_msg">' +
          'Properties changed successfully</div>');
      }
...
function edit_job_dialog(id, i) {
  ...
  if (i === undefined) {
    ...
    _enabled = manage_controllers['items'][i].enabled;
    _description = manage_controllers['items'][i].description;
    _every = manage_controllers['items'][i].every;
  ...
  html +=
    '<div class="form-group">' +
    '<div class="col-12 col-sm-8">' +
    '<label for="job_every">Every</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="job_every" value="' + _every + '" />' +
    '<div class="error_input_msg">Incorrect period format</div>' +
    '</div></div>';
  // html +=
  //   '<div class="col-12 col-sm-6 row-schf">' +
...
manage_controllers['create_units'] = get_unit_props_for_state;
...
function validate_job_dialog() {
  ...
  var period;
  var time;
  $('#job_every').removeClass('device-error');
  if(job.every) {
    ...
    if(every.length < 1 || every.length > 4) {
      $('#job_every').addClass('device-error');
      return false
    }
    if (every.length > 2 && every[every.length-2] != 'at') {
      $('#job_every').addClass('device-error');
      return false;
    }
    if(isInt(every[0])) {
      ...
      if(every[0] <= 0) {
        ...
        if(!singlePeriod.includes(period)) {
          $('#job_every').addClass('device-error');
          return false;
        }
      } else {
        if (!multiPeriod.includes(period)) {
          $('#job_every').addClass('device-error');
          return false;
        } else if (time && !multiWithTime.includes(period)) {
          $('#job_every').addClass('device-error');
          return false;
        }
      }
    ...
    if(time) {
      ...
      if(periodWithPartTime.includes(period) && timeParts.length == 3) {
        $('#job_every').addClass('device-error');
        return false;
      }

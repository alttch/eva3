...
function toggle_log() {
  eva_sfa_log_started = !eva_sfa_log_started;
  if(eva_sfa_log_started) {
    eva_sfa_load_log_entries(false,true);
    $('#log_toggle').removeClass('btn_pause')
      .addClass('btn_resume').html('Pause');
  } else {
    $('#log_toggle').removeClass('btn_resume')
      .addClass('btn_pause').html('Resume');
  }
}
...
function $call(controller_id, func, params, cb_success, cb_error, lock) {
  ...
  eva_sfa_call(
    'management_api_call',
    {i: controller_id, f: func, p: params, t: 7},
    function(data) {
      if (data.code != 0) {
...
function update_remote_cycles(data) {
  ...
  $.each(data, function(k, v) {
    ...
    datarow.append($('<td />').addClass('tbl_v').html(v.iterations));
    tbl.rows.add(datarow);
  });
  tbl.columns.adjust().draw();
}
    ...
    var mngBtns = [{
      ...
      action: function() {
        ...
        $call(id, 'groups', {p: 'LV'}, function(data) {
          manage_controllers['insert_item'] = 'lvars';
          insertGroups(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          ...
          manage_controllers['current_interval'] = setInterval(function() {
            $call(id, 'groups', {p: 'LV'}, function(res) {
            manage_controllers['insert_item'] = 'lvars';
            insertGroups(res);
          }, function() {
      ...
      action: function() {
        ...
        $call(id, 'groups_macro', null , function(data) {
          manage_controllers['insert_item'] = 'macros';
          insertGroups(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          ...
          manage_controllers['current_interval'] = setInterval(function() {
            $call(id, 'groups_macro', null, function(res) {
              manage_controllers['insert_item'] = 'macros';
              insertGroups(res);
            }, function() {
      ...
      action: function() {
        ...
        $call(id, 'groups_cycle', null , function(data) {
          manage_controllers['insert_item'] = 'cycles';
          insertGroups(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          ...
          manage_controllers['current_interval'] = setInterval(function() {
            $call(id, 'groups_cycle', null, function(res) {
              manage_controllers['insert_item'] = 'cycles';
              insertGroups(res);
            }, function() {
      ...
      action: function() {
        ...
        $call(id, 'list_rules', null , function(data) {
          manage_controllers['insert_item'] = 'rules';
          insertRuleList(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
      ...
      action: function() {
        ...
        $call(id, 'list_jobs', null , function(data) {
          manage_controllers['insert_item'] = 'jobs';
          insertJobList(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
    ...
    var mngBtns = [{
      ...
      action: function() {
        ...
        $call(id, 'groups', {p: 'U'}, function(data) {
          manage_controllers['insert_item'] = 'units';
          insertGroups(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          ...
          manage_controllers['current_interval'] = setInterval(function() {
            $call(id, 'groups', {p: 'U'}, function(res) {
              manage_controllers['insert_item'] = 'units';
              insertGroups(res);
            }, function() {
      ...
      action: function() {
        ...
        $call(id, 'groups', {p: 'S'}, function(data) {
          manage_controllers['insert_item'] = 'sensors';
          insertGroups(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          ...
          manage_controllers['current_interval'] = setInterval(function() {
            $call(id, 'groups', {p: 'S'}, function(res) {
              manage_controllers['insert_item'] = 'sensors';
              insertGroups(res);
            }, function() {
              load_error('groups lvars');
      ...
      type: 'owfs',
      action: function() {
        manage_controllers['current_item'] = 'owfs';
        hideManageHolders(uc_block);
        showCurrentHolder('owfs');
        $call(id, 'list_owfs_buses', null, function(data) {
          manage_controllers['insert_item'] = 'owfs';
          insertOWFSList(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          ...
          manage_controllers['current_interval'] = setInterval(function() {
            $call(id, 'list_owfs_buses', null, insertOWFSList, function() {
              load_error('owfs buses');
            });
          }, refreshListTime);
        }, $log);
      }
      ...
      type: 'modbus',
      action: function() {
        manage_controllers['current_item'] = 'modbus';
        hideManageHolders(uc_block);
        showCurrentHolder('modbus');
        $call(id, 'list_modbus_ports', null, function(data) {
          manage_controllers['insert_item'] = 'modbus';
          insertModbusList(data);
          if(manage_controllers['current_interval']) {
            clearInterval(manage_controllers['current_interval']);
            manage_controllers['current_interval'] = null;
          }
          manage_controllers['current_interval'] = setInterval(function() {
            $call(id, 'list_modbus_ports', null, insertModbusList, function() {
              load_error('modbus ports');
            });
          }, refreshListTime);
        }, $log);
      }
      ...
      action: function() {
        ...
        $call(id, api_action, null, function(data) {
          manage_controllers['insert_item'] = 'drivers';
      ...
      action: function() {
        ...
        $call(id, api_action, null, function(data) {
          manage_controllers['insert_item'] = 'modules';
...
function insertGroups(result) {
  var cur_it = manage_controllers['insert_item'] || manage_controllers['current_item'];
  var ih = $('#mng_' + cur_it);
  var ig;
  if($('#mng_' + cur_it + '_group')[0]) {
    ig = $('#mng_' + cur_it + '_group');
    var options = [];
    ...
    $.each(options, function(k,v) {
      if(!result.data.includes(v)) {
        $(ig[0]).find("option[value=" + $.escapeSelector(v) + "]").remove();
        $('#' + cur_it + "_" + $.escapeSelector(v)).remove();
      }
    })
  } else {
    ig = $('<select />', {
      class: 'select_ctrl custom-select',
      id: 'mng_' + cur_it + '_group'
    }).change(function() {
    ...
    ch.append(ig);
    $('<button />', {
      id: 'mng_' + cur_it + '_reload',
      class: 'ctrl_btn ctrl_reload_btn',
      title: 'Reload list'
    }).click(function() {
      var curType = $(this.closest('.ctrl_holder')).find('.mng_btn_active');
      ih.html('');
      manage_controllers['reload_' + cur_it] = false;
      curType.click();
    }).appendTo(ch);
    var f = manage_controllers['create_' + cur_it];
    if(f) {
      $('<button />', {
        id: 'mng_' + cur_it + '_create',
        class: 'ctrl_btn'
      }).click(function() { f(); })
      .html('Create ' + cur_it.slice(0, -1))
      .appendTo(ch);
    }
  }
  loadItemsList();
...
function insertUnitsList(result) {
  ...
  $.each(result.data, function(k, v) {
    ...
    var unit_group = $.escapeSelector(v.group);
    if(!$('#units_' + unit_group)[0]) {
      ig['units_' + v.group] = 
        $('<div />', {
        class: 'group_holder',
        id: 'units_' + v.group
      ...
      ig['units_' + v.group] = $('#units_' + unit_group);
    }
    if(manage_controllers['reload_units'] && manage_controllers['items'][v.oid] && 
      ...
      if($('#'+eoid)[0]) {
        $('#'+eoid).html(unit.html());
        $('[name=unit_status_mng_' + eoid + '][value=' + v.status + ']')
          .attr('checked', true);
      } else {
        ig['units_' + v.group].append(unit);
...
function insertSensorsList(result) {
  ...
  $.each(result.data, function(k, v) {
    ...
    var sensor_group = $.escapeSelector(v.group);
    if(!$('#sensors_' + sensor_group)[0]) {
      ig['sensors_' + v.group] = 
        $('<div />', {
        class: 'group_holder',
        id: 'sensors_' + v.group
      ...
      ig['sensors_' + v.group] = $('#sensors_' + sensor_group);
    }
    if(manage_controllers['reload_sensors'] && manage_controllers['items'][v.oid] && 
      ...
      $('<button />', {
        class: 'item_btn btn_assign',
        onclick: 'get_sensor_props_for_edit("' + 
          manage_controllers["current_controller"] + 
          '", "' + v.oid + '", true)',
        html: 'ASSIGN'
      }).appendTo(sensor_btns);
      ...
      if($('#'+$.escapeSelector(v.oid))[0]) {
        ...
        ig['sensors_' + v.group].append(sensor);
  ...
  hideManageGroupHolders(ih);
  var curId = $('#mng_sensors_group').val();
  showCurrentGroupHolder(curId);
  manage_controllers['reload_sensors'] = true;
}
function insertOWFSList(result) {
  var cur_it = manage_controllers['insert_item'] || manage_controllers['current_item'];
  var ih = $('#mng_' + cur_it);
  var ig;
  if(!manage_controllers['reload_owfs']) {
    ih.html('');
  }
  if($('#' + cur_it + '_list')[0]) {
    ig = $('#' + cur_it + '_list');
    ...
    $('<button />', {
      ...
      create_owfs_dialog();
    }).html('Create OWFS').appendTo(ch);
  }
  var owfs_id_list = [];
  $.each(result.data, function(k, v) {
    owfs_id_list.push('owfs_' + v.id);
    if(manage_controllers['reload_owfs'] && 
        manage_controllers['items']['owfs_' + v.id] && 
          JSON.stringify(v) == 
            JSON.stringify(manage_controllers['items']['owfs_' + v.id])) {
      return true;
    } else {
      manage_controllers['items']['owfs_' + v.id] = v;
      var owfs = $('<div />', {
        id: 'owfs_' + v.id,
        class: 'mng_item_holder'
      });
      ...
      owfs_info.append('Lock: ');
      $('<span />', {
        class: 'item_lock',
        html: v.lock.toString()
      }).appendTo(owfs_info);
      owfs_info.append(', Delay: ');
      $('<span />', {
        class: 'item_delay',
        html: v.delay + 's'
      }).appendTo(owfs_info);
      owfs_info.append('<br>Retries: ');
      $('<span />', {
        class: 'item_retries',
        html: v.retries
      }).appendTo(owfs_info);
      owfs_info.append(', Timeout: ');
      $('<span />', {
        class: 'item_timeout',
        html: v.timeout + 's'
      }).appendTo(owfs_info);
      ...
      $('<button />', {
        class: 'item_btn btn_props',
        onclick: 'create_owfs_dialog("' + v.id + '")',
        title: 'PROPS'
      }).appendTo(owfs_btns);
      $('<button />', {
          class: 'item_btn btn_remove',
          id: 'remove_btn_' + v.id,
          onclick: 'ask_remove_owfs("'+ 
            manage_controllers["current_controller"] + 
            '", "' + v.id + '")',
          title: 'DELETE'
        }).appendTo(owfs_btns);
      if($('#'+$.escapeSelector('owfs_' + v.id))[0]) {
        $('#'+$.escapeSelector('owfs_' + v.id)).html(owfs.html());
      } else {
        ig.append(owfs);
      }
    }
    // eva_sfa_register_update_state(v.id, redraw_owfs);
  });
  $.each(Object.keys(manage_controllers["items"]), function(k, v) {
    if(v.startsWith('owfs_') && !owfs_id_list.includes(v)) {
      $('#'+$.escapeSelector(v)).remove();
      delete manage_controllers["items"][v];
    }
  });
  var curId = $('#mng_' + cur_it + '_group').val();
  manage_controllers['reload_owfs'] = true;
}
function insertModbusList(result) {
  var cur_it = manage_controllers['insert_item'] || manage_controllers['current_item'];
  var ih = $('#mng_' + cur_it);
  var ig;
  if(!manage_controllers['reload_modbus']) {
    ih.html('');
  }
  if($('#' + cur_it + '_list')[0]) {
    ig = $('#' + cur_it + '_list');
    ...
    $('<button />', {
      ...
      manage_controllers['items']['modbus'] = [];
      var curType = $(this.closest('.ctrl_holder')).find('.mng_btn_active');
      manage_controllers['reload_' + cur_it] = false;
      curType.click();
    }).appendTo(ch);
    $('<button />', {
      ...
      create_modbus_dialog();
    }).html('Create modbus').appendTo(ch);
  }
  var modbus_id_list = [];
  $.each(result.data, function(k, v) {
    modbus_id_list.push('modbus_' + v.id);
    if(manage_controllers['reload_modbus'] && 
        manage_controllers['items']['modbus_' + v.id] && 
          JSON.stringify(v) == 
            JSON.stringify(manage_controllers['items']['modbus_' + v.id])) {
      return true;
    } else {
      manage_controllers['items']['modbus_' + v.id] = v;
      var modbus = $('<div />', {
        id: 'modbus_' + v.id,
        class: 'mng_item_holder'
      });
      ...
      modbus_info.append('Lock: ');
      $('<span />', {
        class: 'item_lock',
        html: v.lock.toString()
      }).appendTo(modbus_info);
      modbus_info.append(', Delay: ');
      $('<span />', {
        class: 'item_delay',
        html: v.delay + 's'
      }).appendTo(modbus_info);
      modbus_info.append('<br>Retries: ');
      $('<span />', {
        class: 'item_retries',
        html: v.retries
      }).appendTo(modbus_info);
      modbus_info.append(', Timeout: ');
      $('<span />', {
        class: 'item_timeout',
        html: v.timeout + 's'
      }).appendTo(modbus_info);
      ...
      $('<button />', {
        class: 'item_btn btn_props',
        onclick: 'create_modbus_dialog("' + v.id + '")',
        title: 'PROPS'
      }).appendTo(modbus_btns);
      $('<button />', {
          class: 'item_btn btn_remove',
          id: 'remove_btn_' + v.id,
          onclick: 'ask_remove_modbus("'+ 
            manage_controllers["current_controller"] + 
            '", "' + v.id + '")',
          title: 'DELETE'
        }).appendTo(modbus_btns);
      if($('#'+$.escapeSelector('modbus_' + v.id))[0]) {
        $('#'+$.escapeSelector('modbus_' + v.id)).html(modbus.html());
      } else {
        ig.append(modbus);
      }
    }
    // eva_sfa_register_update_state(v.id, redraw_modbus);
  });
  $.each(Object.keys(manage_controllers["items"]), function(k, v) {
    if(v.startsWith('modbus_') && !modbus_id_list.includes(v)) {
      $('#'+$.escapeSelector(v)).remove();
      delete manage_controllers["items"][v];
    }
  });
  var curId = $('#mng_' + cur_it + '_group').val();
  manage_controllers['reload_modbus'] = true;
}
  ...
  if(result.code === 0 && result.data) {
    ...
    if($('#mng_' + cur_it + '_group')[0]) {
      ...
      ig = $('<select />', {
        ...
        if(this.value == 'lpi') {
          $('#mng_' + cur_it + '_create').hide();
        } else {
          $('#mng_' + cur_it + '_create').show();
        }
        hideManageGroupHolders(ih);
        showCurrentGroupHolder(this.value);
  ...
  if(result.code === 0 && result.data) {
    ...
    if($('#mng_' + cur_it + '_group')[0]) {
      ...
      ig = $('<select />', {
        ...
        if(this.value == 'phi') {
          $('#mng_' + cur_it + '_link').hide();
        } else {
          $('#mng_' + cur_it + '_link').show();
        }
        hideManageGroupHolders(ih);
        showCurrentGroupHolder(this.value);
    ...
    if(type == 'phi') {
      ...
      $('<th />').html('API').appendTo(thead);
      $('<th />', {style: 'width: 95px;'}).html('Action').appendTo(thead);
      ...
      $.each(result.data, function(k, v) {
        ...
        var action_block = $('<td />', {class: 'buttons_block'});
        if(v.can_discover) {
          $('<button />', {
            class: 'item_btn',
            html: 'DISCOVER',
            onclick: 'discover_phi("' + 
              manage_controllers["current_controller"] + '", "' + v.mod + '")'
          }).appendTo(action_block);
        }
        action_block.appendTo(phi);
        phi.appendTo(tbody);
      });
      ...
      $.each(result.data, function(k, v) {
        ...
        if(manage_controllers['reload_phi'] && 
          ...
          $('<button />', {
            class: 'item_btn btn_ports',
            onclick: 'get_phi_ports("' + 
              manage_controllers["current_controller"] + '", "' + v.id + '")',
            html: 'PORTS'
          }).appendTo(phi_btns);
          $('<button />', {
...
function insertLvarsList(result) {
  ...
  $.each(result.data, function(k, v) {
    ...
    var lvar_group = $.escapeSelector(v.group);
    if(!$('#lvars_' + lvar_group)[0]) {
      ig['lvars_' + v.group] = 
        $('<div />', {
        class: 'group_holder',
        id: 'lvars_' + v.group
      ...
      ig['lvars_' + v.group] = $('#lvars_' + lvar_group);
    }
...
function insertMacrosList(result) {
  ...
  $.each(result.data, function(k, v) {
    ...
    var macro_group = $.escapeSelector(v.group);
    if(!$('#macros_' + macro_group)[0]) {
      ig['macros_' + v.group] = 
        $('<div />', {
        class: 'group_holder',
        id: 'macros_' + v.group
      ...
      ig['macros_' + v.group] = $('#macros_' + macro_group);
    }
...
function insertCyclesList(result) {
  ...
  $.each(result.data, function(k, v) {
    ...
    var cycle_group = $.escapeSelector(v.group);
    if(!$('#cycles_' + cycle_group)[0]) {
      ig['cycles_' + v.group] = 
        $('<div />', {
        class: 'group_holder',
        id: 'cycles_' + v.group
    ...
    if(manage_controllers['reload_cycles'] && manage_controllers['items'][v.oid] && 
      ...
      $('<div />', {
        class: 'item_val',
        id: 'cint_' + v.oid,
        html: 'Int = ' + v.interval.toFixed(4) + ', Iter = ' + v.iterations
      }).appendTo(cycle_vals);
      var cycle_btns = $('<div />', {
        class: 'buttons_block'
      }).appendTo(cycle);
      ...
      if($('#' + $.escapeSelector(v.oid))[0]) {
        ...
        ig['cycles_' + v.group].append(cycle);
  ...
  hideManageGroupHolders(ih);
  var curId = $('#mng_cycles_group').val();
  showCurrentGroupHolder(curId);
...
function insertRuleList(result) {
  if(result.code === 0) {
    ...
    if($('#mng_' + cur_it + '_group')[0]) {
      ...
      $('<button />', {
        id: 'mng_' + cur_it + '_diagram',
        class: 'ctrl_btn'
      }).click(function() {
        diagram_dialog();
      }).html('Diagram').appendTo(ch);
...
function item_assign_driver(id, oid) {
  var driverId = $('#item_driver').val();
  var conf = $('#item_configuration').val();
  $call(id, 'assign_driver', {i: oid, d: driverId, c: conf}, function(res) {
    var data = res.data;
    if (data && data.ok) {
      VanillaToasts.create({
        type: 'success',
        text: 'Driver for ' + oid + ' assigned successfully',
        timeout: 2000,
      });
    } else {
      VanillaToasts.create({
        type: 'error',
        text: 'Assign driver command failed for ' + oid + 
          '. Result: ' + data.result,
        timeout: 5000,
      });
    }
    ...
    VanillaToasts.create({
      type: 'error',
      text: 'Server error. Assign driver command failed',
      timeout: 5000,
    });
  });
}
  ...
  $call(id, 'create_sensor', {i: name, g: group}, function(res) {
    ...
    if (data && data.id) {
      ...
      if ('pt' in data && data.pt == 'denied') {
        VanillaToasts.create({
          type: 'error',
          text: 'Current action can not be terminated<br />',
          timeout: 5000,
        });
      } else {
        reload_controller(manage_controllers['current_controller'], 1);
        edit_sensor(manage_controllers['current_controller'], data.oid, true);
      }
    } else {
      VanillaToasts.create({
        type: 'error',
...
function edit_sensor(id, oid, isCreate) {
  var params = sensor_from_edit_dialog();
  $call(id, 'set_prop', {
      i: oid, 
      p:'', 
      v: params
      ...
      if(data && data.ok) {
        ...
        VanillaToasts.create({
          type: 'error',
          text: 'Error changing sensor. Result:' + res,
          timeout: 5000,
        });
      }
    }, function() {
      VanillaToasts.create({
        type: 'error',
        text: 'Server error. Sensor not changed',
        timeout: 5000,
      });
    }
  );
}
function sensor_from_edit_dialog() {
  var res = {}
  res.maintenance_duration = $('#maintenance_duration').val()
    .replace(',', '.') || 0;
  res.notify_events = $('#notify_events').val() || 2;
  var cond = $('#rule_condition').val();
  if (cond == 'equals') {
    var m = $('#sensor_in_range_min')
      .val()
      .replace(',', '.');
    res.value_condition = 'x==' + m;
  } else if (cond == 'range') {
    res.value_condition = 
      $('#sensor_in_range_min_r').val().replace(',', '.') + 
      ($('#sensor_in_range_min_eq').val() == 1 ? '<=' : '<') + 'x' + 
      ($('#sensor_in_range_max_eq').val() == 1 ? '<=' : '<') + 
      $('#sensor_in_range_max_r').val().replace(',', '.');
  } else {
    res.value_condition = "";
  }
  return res;
...
function create_owfs() {
  var id = $('#owfs_id').val();
  var location = $('#owfs_location').val();
  var lock = $('#owfs_lock').val();
  var delay = $('#owfs_delay').val().replace(',', '.') || null;
  var retries = $('#owfs_retries').val() || null;
  var timeout = $('#owfs_timeout').val().replace(',', '.') || null;
  $call(manage_controllers['current_controller'], 'create_owfs_bus', {
    i: id,
    n: location,
    l: lock,
    t: timeout,
    ...
    var timeout = 2000;
    if (data && data.ok) {
      msg = 'OWFS ' + id + ' successfully created';
      $('#' + $.escapeSelector('owfs_' + id)).remove();
    } else {
      msg = 'Unable to create OWFS. Result: ' + data.result;
      popt = 'error';
      timeout = 5000;
    }
    VanillaToasts.create({
...
function remove_owfs(controller, id) {
  $call(controller, 'destroy_owfs_bus', {i: id}, function(res) {
    var data = res.data;
    var msg = '';
    var popt = 'success';
    var timeout = 2000;
    if (data && data.ok) {
      msg = 'OWFS ' + id + ' successfully deleted';
      $('#' + $.escapeSelector('owfs_' + id)).remove();
    } else {
      msg = 'Unable to delete OWFS. Result: ' + data.result;
      popt = 'error';
      timeout = 5000;
    }
    VanillaToasts.create({
...
function create_modbus() {
  var id = $('#modbus_id').val();
  var paremeters = $('#modbus_parameters').val();
  var lock = $('#modbus_lock').val();
  var delay = $('#modbus_delay').val().replace(',', '.') || null;
  var retries = $('#modbus_retries').val() || null;
  var timeout = $('#modbus_timeout').val().replace(',', '.') || null;
  $call(manage_controllers['current_controller'], 'create_modbus_port', {
    i: id,
    p: paremeters,
    l: lock,
    t: timeout,
    ...
    var timeout = 2000;
    if (data && data.ok) {
      msg = 'Modbus ' + id + ' successfully created';
      $('#' + $.escapeSelector('modbus_' + id)).remove();
    } else {
      msg = 'Unable to create modbus. Result: ' + data.result;
      popt = 'error';
      timeout = 5000;
    }
    VanillaToasts.create({
...
function remove_modbus(controller, id) {
  $call(controller, 'destroy_modbus_port', {i: id}, function(res) {
    var data = res.data;
    var msg = '';
    var popt = 'success';
    var timeout = 2000;
    if (data && data.ok) {
      msg = 'Modbus ' + id + ' successfully deleted';
      $('#' + $.escapeSelector('modbus_' + id)).remove();
    } else {
      msg = 'Unable to delete modbus. Result: ' + data.result;
      popt = 'error';
      timeout = 5000;
    }
    VanillaToasts.create({
...
function discover_phi(controller, mod) {
  $call(controller, 'phi_discover', {m: mod, w: 5}, function(res) {
    var data = res.data;
    if (data) {
      if(Array.isArray(data) && data.length) {
        module_discover_dialog(controller, mod, data);
      } else {
        VanillaToasts.create({
          type: 'info',
          text: 'Discover command for <b>' + mod + 
            '</b> returned an empty array',
          timeout: 5000,
        });
      }
    } else {
      VanillaToasts.create({
        type: 'error',
        text: 'Server error. Discover command failed for ' + mod +
          '.Result: ' + res,
        timeout: 5000,
      });
    }
    ...
    VanillaToasts.create({
      type: 'error',
      text: 'Server error. Discover command failed',
      timeout: 5000,
    });
  });
}
...
function redraw_unit(v) {
  ...
  $('[name=unit_status_mng_' + eoid + ']').removeAttr('checked');
  $('[name=unit_status_mng_' + eoid + '][value=' + v.status + ']').attr('checked', true);
  var eb = $('[id="btn_enable_' + eoid + '"]');
  if (eva_sfa_states[v.oid].action_enabled) {
    $('[name=unit_status_mng_' + eoid + ']').removeAttr('disabled');
    eb.html('ENABLED');
    ...
    $('[name=unit_status_mng_' + eoid + ']').prop('disabled', 'disabled');
  }
}
...
function redraw_cycle_state(v) {
  ...
  $('#cint_' + eoid).html('Int = ' + 
    eva_sfa_states[v.oid].interval.toFixed(4) + ', Iter = ' + 
...
function get_unit_props_for_edit(id, oid, isAssign) {
  $call(id, 'list_props', { i: oid }, function(res) {
    var data = res.data;
    if(data && res.code === 0) {
      if(isAssign) {
        drivers_list_for_item_assign(id, oid, data);
      } else {
...
function unit_props_dialog(id, oid, data) {
  ...
  if(!id) {
    ...
    html += 
      ...
      'data-target="#un_adv" aria-expanded="false">Advanced</div>' +
      '<div class="collapse controller_holder" id="un_adv">';
  }

  html +=
    // line 1
    '<div class="form-group row">' +
    '<div class="col-12 col-sm-6 col-801-4">' +
    bool_line('action_allow_termination', 'Action allow termination', 
      data, [false, true]) +
    '</div>' +
    // line 2
    '<div class="col-12 col-sm-6 col-801-4 row-schf">' +
    bool_line('action_always_exec', 'Action always exec', 
      data, [false, true]) +
    '</div>' +
    // line 4
    '<div class="col-12 col-sm-6 col-801-4 row-thrd">' +
  ...
  html += labels + '" /></div></div>' +
    ...
    '</div>' +
    // line 24
    '<div class="col-12 col-sm-4 col-801-4 row-thrd">' +
    ...
    '<select class="form-control custom-select" id="rule_condition"' +
  ...
  $popup('!confirm', (id ? 'Edit unit ' + oid : 'Create unit'), 
    html, {
      wclass: 'edit_unit_dialog_window',
      btn1:'OK',
      btn2: 'CANCEL',
      btn1a: function() {
        if(id) {
...
function drivers_list_for_item_assign(id, oid, itemData) {
  $call(id, 'list_drivers', null, function(res) {
    var data = res.data;
    var conf = "";
    $.each(itemData.update_driver_config, function(k, v) {
      conf += "," + k + "=" + v;
    });
    var curDriver = {
      id: itemData.update_exec ? itemData.update_exec.substr(1) : "",
      conf: conf ? conf.substr(1) : ""
    }
    if(data && res.code === 0) {
      unit_assign_dialog(id, oid, curDriver, data);
    } else {
      VanillaToasts.create({
        type: 'error',
        text: 'Server error. Unable to get drivers list. Result: ' + data.result,
        timeout: 5000,
      });
    }
  }, function() {
    VanillaToasts.create({
      type: 'error',
      text: 'Server error. Unable to get drivers list',
      timeout: 5000,
    });
  });
}
function unit_assign_dialog(id, oid, curDriver, data) {
  var html = '<form class="form-horizontal" id="item_assign_driver">';
  html +=
    '<div class="form-group">' +
    '<label for="item_driver">Driver</label>' +
    '<select id="item_driver" class="form-control custom-select">' +
    '<option value="">none</option>';
  $.each(data, function(k, v) {
      html += '<option value="' + v.id + '"';
      if(curDriver.id == v.id) {
        html += ' selected';
      }
      html += '>' + v.id + '</option>';
    });
  html += '</select></div>' + 
    '<div class="form-group">' +
    '<label for="item_configuration">Configuration</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="item_configuration" value="' + curDriver.conf + '" /></div></form>';
  $popup('confirm', ('Assign driver for ' + oid), 
    html, {
      btn1:'OK',
      btn2: 'CANCEL',
      btn1a: function() {
        item_assign_driver(id, oid);
      }
    }
  );
}
...
function get_sensor_props_for_edit(id, oid, isAssign) {
  $call(id, 'list_props', { i: oid }, function(res) {
    var data = res.data;
    if(data && res.code === 0) {
      if(isAssign) {
        ...
        edit_sensor_dialog(id, oid, data);
      }
    } else {
      VanillaToasts.create({
        type: 'error',
        text: 'Server error. Unable to get sensor props. Result: ' + data.result,
        timeout: 5000,
      });
    }
  }, function() {
    VanillaToasts.create({
      type: 'error',
      text: 'Server error. Unable to get sensor props',
      timeout: 5000,
    });
  });
}
function edit_sensor_dialog(id, oid, data) {
  var _in_range_min = '';
  var _in_range_max = '';
  var _in_range_min_eq = false;
  var _in_range_max_eq = false;
  ...
  if(!oid) {
    html +=
      ...
      '<input class="form-control" type="text" size="5"' +
      'id="sensor_name" value="" /></div>';
    html += 
      '<div class="btn collapse_btn" data-toggle="collapse" '+
      'data-target="#sn_adv" aria-expanded="false">Advanced</div>' +
      '<div class="collapse controller_holder" id="sn_adv">';
  }
  html += 
    // line 1
    '<div class="form-group">' +
  ...
  // equals
    ...
    'x&nbsp;==</span></div>' +
    '<input class="form-control" type="text" size="5"' +
    'id="sensor_in_range_min" value="' + _in_range_min + '"></div>';
  html += 
  // line 3.5
  // range
    '<div class="col-12 col-sm-8 input-group row-schf" ' +
    'id="d_rule_cond_range">' +
    '<input class="form-control" type="text" size="5"' +
    'id="sensor_in_range_min_r" value="' + _in_range_min + '">' +
    '<select class="form-control custom-select" id="sensor_in_range_min_eq">' +
    '<option value="0">&lt;</option>'+
    '<option value="1"' + (_in_range_min_eq ? ' selected' : '') +
    '>&lt;=</option></select>' +
    '<span class="input-group-text" id="l_unit_cond_eq">x</span>' +
    '<select class="form-control custom-select" id="sensor_in_range_max_eq">' +
    '<option value="0">&lt;</option>' +
    '<option value="1"' +(_in_range_max_eq ? ' selected' : '') +
    '>&lt;=</option></select>' +
    '<input class="form-control" type="text" size="5"' +
    'id="sensor_in_range_max_r" value="' + _in_range_max + '"></div></div>';
  if(!oid) {
    html += '</div>';
  }
  html += '</form>';
  $popup('confirm', (oid ? 'Edit' : 'Create') + ' sensor',
    html, {
      btn1: 'OK',
      btn2: 'CANCEL',
      btn1a: function() {
...
function create_owfs_dialog(id) {
  var html =
    '<form class="form-horizontal">' +
    // line 1
    '<div class="form-group">' +
    '<label class="control-label" for="owfs_id">Name:</label>' +
    '<label class="form_field_required">*</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="owfs_id" value="' + 
    (id ? manage_controllers['items']['owfs_'+id].id : "") + '" /></div>' +
    // line 2
    '<div class="form-group">' +
    '<label class="control-label" for="owfs_location">Location:</label>' +
    '<label class="form_field_required">*</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="owfs_location" value="' + 
    (id ? manage_controllers['items']['owfs_'+id].location : "") + 
    '" /></div>' +
    // line 3
    '<div class="form-group row">' +
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="owfs_lock">Lock:</label>' +
    '<select class="form-control custom-select" id="owfs_lock" value="">' +
    '<option value="false"' + 
    (id && !manage_controllers['items']['owfs_'+id].lock ? " selected" : "") + 
    '>False</option><option value="true"' + 
    (id && manage_controllers['items']['owfs_'+id].lock ? " selected" : "") + 
    '>True</option></select></div>' +
    // line 4
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="owfs_delay">Delay:</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="owfs_delay" value="' + 
    (id ? manage_controllers['items']['owfs_'+id].delay : "") + 
    '" /></div></div>' +
    // line 5
    '<div class="form-group row">' +
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="owfs_retries">Retries:</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="owfs_retries" value="' + 
    (id ? manage_controllers['items']['owfs_'+id].retries : "") + 
    '" /></div>' +
    // line 6
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="owfs_timeout">Timeout:</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="owfs_timeout" value="' + 
    (id ? manage_controllers['items']['owfs_'+id].timeout : "") + 
    '" /></div></div></form>';
  $popup('confirm', 'Create sensor',
    html, {
      ...
      va: validate_owfs_dialog
    }
  );
  $('#owfs_id').focus();
}
function ask_remove_owfs(controller_id, id) {
  $popup('warning', 'Remove ' + id, '<div class="content_msg">' +
    'Do you realy want to delete ' + id + '?</div>', {
      btn1: 'OK',
      btn1a: function() {
        remove_owfs(controller_id, id);
      },
      btn2: 'CANCEL'
    }
  );
...
function create_modbus_dialog(id) {
  var html =
    '<form class="form-horizontal">' +
    // line 1
    '<div class="form-group">' +
    '<label class="control-label" for="modbus_id">Name:</label>' +
    '<label class="form_field_required">*</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="modbus_id" value="' + 
    (id ? manage_controllers['items']['modbus_'+id].id : "") + 
    '" /></div>' +
    // line 2
    '<div class="form-group">' +
    '<label class="control-label" for="modbus_parameters">' +
    'Parameters:</label>' +
    '<label class="form_field_required">*</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="modbus_parameters" value="' + 
    (id ? manage_controllers['items']['modbus_'+id].params : "") + 
    '" /></div>' +
    // line 3
    '<div class="form-group row">' +
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="modbus_lock">Lock:</label>' +
    '<select class="form-control custom-select" id="modbus_lock" value="">' +
    '<option value="false"' + 
    (id && !manage_controllers['items']['modbus_'+id].lock ? " selected" : "") + 
    '>False</option><option value="true"' + 
    (id && manage_controllers['items']['modbus_'+id].lock ? " selected" : "") + 
    '>True</option></select></div>' +
    // line 4
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="modbus_delay">Delay:</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="modbus_delay" value="' + 
    (id ? manage_controllers['items']['modbus_'+id].delay : "") + 
    '" /></div></div>' +
    // line 5
    '<div class="form-group row">' +
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="modbus_retries">Retries:</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="modbus_retries" value="' + 
    (id ? manage_controllers['items']['modbus_'+id].retries : "") + 
    '" /></div>' +
    // line 6
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="modbus_timeout">Timeout:</label>' +
    '<input class="form-control" type="text" size="5"' +
    'id="modbus_timeout" value="' + 
    (id ? manage_controllers['items']['modbus_'+id].timeout : "") + 
    '" /></div></div></form>';
  $popup('confirm', 'Create sensor',
    html, {
      btn1: 'CREATE',
      ...
      va: validate_modbus_dialog
    }
  );
  $('#modbus_id').focus();
}
function ask_remove_modbus(controller_id, id) {
  $popup('warning', 'Remove ' + id, '<div class="content_msg">' +
    'Do you realy want to delete ' + id + '?</div>', {
      btn1: 'OK',
      btn1a: function() {
        remove_modbus(controller_id, id);
      },
      btn2: 'CANCEL'
    }
  );
...
function driver_load_dialog(controller, id, data) {
  ...
  if(!id) {
    html +=
      ...
      '<select id="form_mod_phi" class="form-control custom-select">';
    ...
    html += '</select>' +
      ...
      '<select id="form_mod_lpi_module" class="form-control custom-select" ' +
      'value="" onchange="change_config_block(\'' + controller + 
      '\', \'lpi\', ' + 'this.value)">';
...
function change_config_block(controller, type, mod, data) {
  $call(controller, 'modhelp_' + type, {m: mod, c:"cfg"}, function(res) {
    if(res && res.data) {
      html = '';
      var unchaned = data ? data['!load'] : null;
      $.each(res.data, function(k, v) {
        ...
        html += '<div class="info_block_btn" data-action="vanillatoast" ' +
          ...
          if(v.type == 'bool') {
            html += '<select class="form-control custom-select" ' +
              'id="form_mod_' + v.name + '"' + (unchaned?' disabled':'') + '>';
            if(!v.required)
            ...
            html +=
              ...
              v.name + '" value="' + 
              (unchaned && unchaned[v.name] ? unchaned[v.name] : 
                data && data[v.name] ? data[v.name] : "") + '"' + 
              (unchaned && unchaned[v.name] ? ' readonly' : '') + '/>';
          }
        if(v.type == 'list')
...
function get_phi_ports(controller, id) {
  $call(controller, 'get_phi_ports', { i: id }, function(res) {
    var data = res.data;
    if(data && res.code === 0) {
      var msg = '<table class="table ports-table">' +
        '<tr><th>Port</th><th>Name</th><th>Description</th></tr>';
      for(var i in data) {
        msg += '<tr><td>' + data[i].port + '</td><td>' + data[i].name + 
          '</td><td>' + data[i].description + '</td></tr>';
      }
      msg += '</table>';
      $popup('info', 'Ports for ' + id, msg);
    } else {
      VanillaToasts.create({
        type: 'error',
        text: 'Server error. Unable to get module ports. Result: ' + 
          data.result,
        timeout: 5000,
      });
    }
...
function module_load_dialog(controller, id, data) {
  ...
  if(!id) {
    html +=
      ...
      '<select id="phi_module" class="form-control custom-select" value="" ' +
...
var discover_data = {};
function module_discover_dialog(controller, mod, data) {
  var cols = [];
  discover_data = data;
  if(data[0]["!opt"]) {
    cols = data[0].value;
  ...
  for(var i in data) {
    if(!data[i]["!opt"]) {
      html += '<div class="form-group">';
      for(var j in cols) {
        html += '<div style="width: 100%">' + cols[j] + ' : ' + 
          data[i][cols[j]] + '</div>';
      }
      if(data[i]["!load"]) {
        html += '<button onclick="module_load_custom_dialog(\'' + controller +
        '\', \'' + mod + '\', ' + +i +')" class="btn_load">LOAD</button>';
      }
      html += '</div>';
    }
  }
  html += '</form>';
  $popup('confirm', 'Discover module', html, 'OK');
}
function module_load_custom_dialog(controller, mod, i) {
  var html = '<form class="form-horizontal">' +
    // line 1
    '<div class="form-group">' +
    '<label class="control-label"' +
    ...
    '<label class="control-label"' +
    ' for="phi_module">PHI module:</label>' +
    '<input class="form-control" type="text" size="5" id="phi_module" ' +
    ' value="' + mod + '" readonly/></div>';
  html += '<div id="cfg_block"></div>';
  html += '</form>';
  $popup('confirm', 'Load module',
    html, {
      btn1: 'OK',
      btn2: 'CANCEL',
      btn1a: function() {
  ...
  $('#module_id').focus();
  change_config_block(controller, 'phi', mod, discover_data[i]);
}
...
function edit_lvar_dialog(id, oid, props) {
  ...
  html +=
    ...
    'value="' + (props ? props.notify_events : 2) + '"/></div>' +
    '<div class="col-12 col-sm-6">' +
...
function edit_macro_dialog(id, oid, props) {
  ...
  $popup('confirm', (id ? 'Edit macro ' + oid : 'Create macro'),
    html, {
      btn1: 'OK',
      btn2: 'CANCEL',
      btn1a: function() {
...
function diagram_dialog(id, oid, props) {
  $('head').append($('<script />', {
    type: 'text/javascript',
    html: 'mxBasePath = "//jgraph.github.io/mxgraph/javascript/src";'
  }));
  var mxCl = document.createElement('script');
  mxCl.src = "//jgraph.github.io/mxgraph/javascript/mxClient.min.js";
  document.head.append(mxCl);
  mxCl.onload = function() {
  ...
  var html =
    '<script src="js/diagram.js" type="text/javascript" charset="utf-8"></script>' +
    '<script src="js/diagram_sidebar.js" type="text/javascript" charset="utf-8"></script>' +
    '<script src="js/diagram_toolbar.js" type="text/javascript" charset="utf-8"></script>' +
    '<div id="diagram_editor">' + 
    '<div id="title" style="position: absolute;"></div>' +
    '<div id="toolbarContainer" class="toolbar_container"></div>' +
    '<div id="sidebarContainer" class="sidebar_container"></div>' +
    '<div id="graphContainer" class="graph_container"></div></div>';
  $popup('confirm', 'Edit diagram',
    html, {
      wclass: 'diagram_editor'
    }
  );
}
...
function edit_cycle_dialog(id, oid, props) {
  ...
  html += '"/></div></div>' +
    ...
    '<select class="form-control custom-select" id="cycle_macro" ' +
...
function edit_rule_dialog(id, i) {
  ...
  html +=
    ...
    '<select class="form-control custom-select" id="rule_for_prop">';
  ...
  html +=
    ...
    '<select class="form-control custom-select" id="rule_item_type">';
  ...
  html +=
    ...
    '<select class="form-control custom-select" id="rule_for_initial">';
  ...
  html +=
    ...
    '<select class="form-control custom-select" id="rule_macro" ' +
    'onchange="type_macro(this)">' +
    '<option value="">---</option>' +
    '<option value="-1"';
  var custom_macro = false;
  var possible_macro_names = [];
  for(var name in manage_controllers["items"]) {
    if(name.startsWith('lmacro')) {
      possible_macro_names.push(name);
      possible_macro_names.push(manage_controllers["items"][name].full_id);
      possible_macro_names.push(manage_controllers["items"][name].id);
    }
  }
  if(_macro && _macro != null && 
      !possible_macro_names.includes(_macro)) {
    html += ' selected';
    custom_macro = true;
  }
  ...
  for(var m in manage_controllers["items"]) {
    if(m.startsWith('lmacro')) {
      html += '<option value="' + m + '"';
      if(_macro === m || _macro === manage_controllers["items"][m].full_id || 
          _macro === manage_controllers["items"][m].id) 
        html += ' selected ';
      html += '>' + manage_controllers["items"][m].full_id + '</option>';
    }
  }
  html += '</select></div>';
...
function edit_job_dialog(id, i) {
  ...
  html +=
    ...
    '<div class="col-12 col-sm-6">' +
    '<label for="rule_macro">Macro</label>' +
    '<select class="form-control custom-select" id="job_macro" ' +
    'onchange="type_macro(this)">' +
    '<option value="">---</option>' +
    '<option value="-1"';
  var custom_macro = false;
...
manage_controllers['create_sensors'] = edit_sensor_dialog;
...
function validate_sensor_dialog() {
  var _v = true;
  $('#maintenance_duration').removeClass('device-error');
  $('#notify_events').removeClass('device-error');
  $('#rule_in_range_min_r').removeClass('device-error');
  ...
  var sensor = sensor_from_edit_dialog();

  if(sensor.maintenance_duration && !isNumeric(sensor.maintenance_duration)) {
    $('#maintenance_duration').addClass('device-error');
    $('#maintenance_duration').focus();
    _v = false;
  }
  if(sensor.notify_events && !isInt(sensor.notify_events)) {
    $('#notify_events').addClass('device-error');
    $('#notify_events').focus();
    _v = false;
  }
  if ($('#rule_condition').val() == 'range') {
    if ($('#sensor_in_range_min_r').val() != '' &&
        (!isNumeric($('#sensor_in_range_min_r').val()) ||
          (isNumeric($('#sensor_in_range_min_r').val()) &&
            isNumeric($('#sensor_in_range_max_r').val()) &&
              Number($('#sensor_in_range_min_r').val()) >= 
                Number($('#sensor_in_range_max_r').val()))
    )) {
      $('#sensor_in_range_min_r').addClass('device-error');
      if (_v) $('#sensor_in_range_min_r').focus();
      _v = false;
    }
    if (($('#sensor_in_range_max_r').val() != '' && 
        !isNumeric($('#sensor_in_range_max_r').val()) ||
          (isNumeric($('#sensor_in_range_min_r').val()) && 
            isNumeric($('#sensor_in_range_max_r').val()) &&
              Number($('#sensor_in_range_min_r').val()) >= 
      ...
      $('#sensor_in_range_max_r').addClass('device-error');
      if (_v) $('#sensor_in_range_max_r').focus();
      _v = false;
    }
  } else if($('#rule_condition').val() == 'equals' &&
    ($('#sensor_in_range_min').val() == '' || 
      !isNumeric($('#sensor_in_range_min').val()))) {
    $('#sensor_in_range_min').addClass('device-error');
    if (_v) $('#sensor_in_range_min').focus();
    _v = false;
  }
  return _v;
}
function validate_owfs_dialog() {
  var res = true;
  var id = $('#owfs_id').removeClass('device-error');
  var location = $('#owfs_location').removeClass('device-error');
  var delay = $('#owfs_delay').removeClass('device-error');
  var retries = $('#owfs_retries').removeClass('device-error');
  var timeout = $('#owfs_timeout').removeClass('device-error');
  if(!id.val().trim()) {
    id.addClass('device-error');
    res = false;
  }
  if(!location.val().trim()) {
    location.addClass('device-error');
    res = false;
  }
  if (delay.val() && !isNumeric(delay.val().replace(',','.'))) {
    delay.addClass('device-error');
    res = false;
  }
  if (retries.val() && !isInt(retries.val())) {
    retries.addClass('device-error');
    res = false;
  }
  if(timeout.val() && !isNumeric(timeout.val().replace(',', '.'))) {
    timeout.addClass('device-error');
    res = false;
  }
  return res;
}
function validate_modbus_dialog() {
  var res = true;
  var id = $('#modbus_id').removeClass('device-error');
  var parameters = $('#modbus_parameters').removeClass('device-error');
  var delay = $('#modbus_delay').removeClass('device-error');
  var retries = $('#modbus_retries').removeClass('device-error');
  var timeout = $('#modbus_timeout').removeClass('device-error');
  if(!id.val().trim()) {
    id.addClass('device-error');
    res = false;
  }
  if(!parameters.val().trim()) {
    parameters.addClass('device-error');
    res = false;
  }
  if (delay.val() && !isNumeric(delay.val().replace(',','.'))) {
    delay.addClass('device-error');
...
function show_debug_info(res) {
  ...
  if (debug_mode[manage_controllers['current_controller']]) {
    $('#i_debug_' + eoid).addClass('debug_active');
    $('#debug_' + eoid).attr('checked', 'checked');
  } else {
    $('#i_debug_' + eoid).removeClass('debug_active');
...
var tbars = [
  ...
  },
  {
    name: 'Cycles',
    id: 'remote-cycles',
    cols: ['Controller', 'Cycle ID', 'Status', 'Interval', 'Iterations'],

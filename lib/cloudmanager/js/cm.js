  ...
  if(!uc_block.find('.mng_btn_holder')[0]) {
    ...
    var mngBtns = [{
      ...
      action: function() {
        ...
        // $call(id, 'list_drivers', null, function(data) {
        //   insertDriversList(data);
        //   if(manage_controllers['current_interval']) {
        //     clearInterval(manage_controllers['current_interval']);
        //     manage_controllers['current_interval'] = null;
        //   }
        //   // manage_controllers['current_interval'] = setInterval(function() {
        //   //   $call(id, 'list_phi_mods', null, insertModules, function() {
...
function load_driver(id) {
  var params = {};
  params.i = $('#driver_lpi').val().trim();
  params.m = $('#driver_lpi_module').val().trim();
  params.p = $('#driver_phi').val().trim();
  params.c = {};
  var inputs = $('#cfg_block [id^="driver_"');
  $.each(inputs, function(k, v) {
    if(v.value) params.c[v.id.substr(7)] = v.value;
  });
...
function set_driver(controller, id) {
  ...
  params.p = '';
  params.v = {};
  // var cfg = $('#driver_config').val().replace(/\s/g, '').split(',');
  // $.each(cfg, function(k, v) {
  //   if(v.length > 0 && v.split('=').length == 2) {
  //     params.v[v.split('=')[0]] = v.split('=')[1];
  //   }
  // });
  var inputs = $('#cfg_block [id^="driver_"');
  $.each(inputs, function(k, v) {
    params.v[v.id.substr(7)] = v.value;
  });
...
function set_phi(controller, id) {
  ...
  params.p = '';
  params.v = {};
  var cfg = $('#phi_config').val().replace(/\s/g, '').split(',');
  $.each(cfg, function(k, v) {
    if(v.length > 0 && v.split('=').length == 2) {
      params.v[v.split('=')[0]] = v.split('=')[1];
    }
...
function driver_load_dialog(controller, id, data) {
  ...
  if(!id) {
    html +=
      ...
      '<div class="form-group">' +
      '<label class="control-label"' +
      ' for="driver_lpi">LPI ID:</label>' +
      '<input class="form-control" type="text" size="5"' +
      'id="driver_lpi" value="" /></div>' +
      // line 2
      '<div class="form-group">' +
      '<label class="control-label"' +
      ' for="driver_lpi_module">LPI module:</label>' +
      '<select id="driver_lpi_module" class="form-control" value="" ' +
      'onchange="change_config_block(\'' + controller + '\', this.value)">';
    ...
    html += '</select></div>' + 
      ...
      '<div class="form-group">' +
      '<label class="control-label"' +
      ' for="driver_phi">PHI ID:</label>' +
      '<input class="form-control" type="text" size="5"' +
      'id="driver_phi" value="" /></div>';
  }
  // html +=
  //   // line 4
  //   '<div class="form-group">' +
  //   '<label class="control-label"' +
  //   ' for="driver_config">Driver config:</label>' +
  //   '<input id="driver_config" class="form-control" type="text" size="5"' +
  //   'onfocus="this.selectionStart=this.selectionEnd=10000" value="';
  // if(data) {
  //   $.each(data.cfg, function(k, v) {
  //     html += k + '=' + v + ', ';
  //   });
  // }
  // html += 
  //   '"/><div class="input_helper">Coma separated args</div></div>';

  // var i = 0;
  // if(data && data.cfg.length > 0) {
  //   $.each(data.cfg ,function(k, v) {
  //     html += cfgblock(i++, data);
  //   });
  // } else {
  //   html += cfgblock(i++);
  // }

  // html += '<div onclick="addCfg(' + i + ')">Add cfg</div>';
  html += '<div id="cfg_block"></div>';
  html += '</form>';
  var title = id ? 'Edit driver ' + id : 'Create driver';
    // '<div class="info_block_btn" data-toggle="tooltip" data-html="true" data-placement="bottom" ' + 
    // 'title="<table><thead><tr><th>Name</th><th>Type</th><th>Required</th></tr></thead>' +
    // '<tbody><tr><td>host</td><td>str</td><td>true</td></tr><tr><td>community</td><td>str</td><td>false</td></tr><tr><td>update</td><td>float</td><td>false</td></tr></tbody>" ' +
    // '>?</div>';
  $popup('confirm', title,
    html, {
      btn1: 'OK',
  ...
  if(id) {
    change_config_block(controller, data.mod, data.cfg);
    // $('#driver_config').focus();
  } else {
    $('#driver_id').focus();
    change_config_block(controller, $('#driver_lpi_module').val());
  }
  /*for(var k = 0; k < i; k++)
    module_form_condition_switch("config_type", k);*/
  // $('[data-toggle="tooltip"]').tooltip();
}
function change_config_block(controller, lpi, data) {
  $call(controller, 'modhelp_lpi', {m: lpi, c:"cfg"}, function(res) {
    if(res && res.data) {
      html = '';
      $.each(res.data, function(k, v) {
        html += '<div class="form-group" style="position:relative">' +
          '<label class="control-label"' +
          ' for="driver_' + v.name + '">' + v.name + ':</label>' +
          '<div class="info_block_btn" data-toggle="tooltip" data-html="true"' +
          ' data-placement="bottom" title="<table><tr><th>Name</th>' +
          '<td>' + v.name + '</td></tr><tr><th>Type</th><td>' + v.type + 
          '</td></tr><tr><th>Required</th><td>' + v.required + 
          '</td></tr><tr><th>Help</th><td>' + v.help + '</td></tr>">?</div>' +
          '<input class="form-control" type="text" size="5" data-type="' +
          v.type + '" data-required="' + v.required + '" id="driver_' + 
          v.name + '" value="' + 
...
/*function addCfg(i) {
  event.target.outerHTML = cfgblock(i++) + 
    '<div onclick="addCfg(' + i + ')">Add cfg</div>';
}
function cfgblock(i) {
  var html =
    '<div class="btn collapse_btn" data-toggle="collapse" '+
    'data-target="#cfg_' + i + '" aria-expanded="' + (i > 0 ? false : true) + 
    '">CFG ' + i + '</div><div class="collapse controller_holder' + 
    (i == 0 ? ' show' : '') + '" id="cfg_' + i + '">' +
    //line 1
    '<div class="form-group">' +
    '<label class="control-label" for="config_name_' + i + '">Name:</label>' +
    '<input id="config_name_' + i + '" class="form-control" type="text" size="5" />' +
    '</div>' +
    //line 2
    '<div class="form-group">' +
    '<label class="control-label" for="config_help_' + i + '">Help:</label>' +
    '<input id="config_help_' + i + '" class="form-control" type="text" size="5" />' +
    '</div>' +
    //line 3
    '<div class="form-group row">' +
    '<div class="col-12 col-sm-6">' +
    '<label class="control-label" for="config_type_' + i + '">Type:</label>' +
    '<select id="config_type_' + i + '" class="form-control" ' +
    'onchange="module_form_condition_switch(\'config_type\', ' + i + ')">';
  $.each(allow_types, function(k, v) {
    html += '<option value="' + v + '">' + v + '</option>';
  });
  html += '</select></div>';
  html += '<div class="col-12 col-sm-6 row-schf" id="list_type_' + i + '">' +
    '<label class="control-label" for="config_type_list_' + i + '">List of:</label>' +
    '<select id="config_type_list_' + i + '" class="form-control">';
  $.each(allow_types, function(k, v) {
    if(v == 'list') return;
    html += '<option value="' + v + '">' + v + '</option>';
  });
  ...
  html += 
    '<div class="col-12 col-sm-6 row-schf">' +
    '<label class="control-label" for="config_required_' + i + '">Required:</label>' +
    '<select id="config_required_' + i + '" class="form-control" value="true">' +
...
function module_load_dialog(controller, id, data) {
  ...
  if(!id) {
    html +=
      ...
      html += '</select></div>';
      // '<input class="form-control" type="text" size="5"' +
      // 'id="phi_module" value="" /></div>';
  }
  html +=
    // line 3
    '<div class="form-group">' +
    '<label class="control-label"' +
    ' for="phi_config">PHI configuration:</label>' +
    '<input id="phi_config" class="form-control" type="text" size="5"' +
    'onfocus="this.selectionStart=this.selectionEnd=10000" value="';
  if(data) {
    $.each(data.cfg, function(k, v) {
      html += k + '=' + v + ', ';
  ...
  html += 
    '"/><div class="input_helper">Coma separated args</div></div></form>';
  $popup('confirm', id ? 'Set module ' + id : 'Load module',
    html, {
      ...
      va: function() {
        if(!id) {
          return validate_load_phi_dialog();
        } else {
          return true
        }
      }
...
function edit_job_dialog(id, i) {
  ...
  // html +=
  //   '<div class="col-12 col-sm-6 row-schf">' +
  //   '<label for="job_last">Last</label>' +
  //   '<input class="form-control" type="text" size="5"' +
  //   'id="job_last" value="' + _last + '" /></div></div>';
  // row 3
  html +=
    '<div class="form-group">' +
    '<div class="col-12 col-sm-6">' +
...
/*function module_form_condition_switch(id, i) {
  if ($('#'+id + '_' + i).val() == 'list') {
    $('#list_type_' + i).show();
  } else {
    $('#list_type_' + i).hide();
...
function validate_load_driver_dialog(id) {
  var _v = true;
  if(!id) {
    $('[id^=driver_').removeClass('device-error');
    var i = $('#driver_lpi').val().trim();
    if(!i) {
      $('#driver_lpi').addClass('device-error');
      $('#driver_lpi').focus();
      _v = false;
    }
    var i = $('#driver_phi').val().trim();
    if(!i) {
      $('#driver_phi').addClass('device-error');
      $('#driver_phi').focus();
      _v = false;
    }
  }
  $.each($('[id^=driver_'), function(k, v) {
...
function validate_load_phi_dialog() {
  var _v = true;
...
function job_from_edit_dialog() {
  ...
  // job.last = $('#job_last').val();
  // if (job.last == '') job.last = null;

...
function reload_remote_data() {
  eva_sfa_call('list_remote', null, update_remote, function() {
    load_error('remote data');
  });
}

function reload_remote_cycles_data() {
  eva_sfa_call('list_cycles', null, update_remote_cycles, function() {
    load_error('cycles');
  });
}

function update_remote(data) {
  $('#tbl-remote-items-body')
    .find('tr')
    .remove()
    .end();
  $.each(data, function(k, v) {
    var datarow = $('<tr />');
    if (k % 2) {
      datarow.addClass('tbl-items-odd');
    } else {
      datarow.addClass('tbl-items-even');
    }
    datarow.addClass('item-status-' + v.status);
    datarow.append(
      $('<td />')
        .addClass('vtop')
        .prop('width', 190)
        .html(v.controller_id)
    );
    datarow.append(
      $('<td />')
        .addClass('vtop')
    ...
    $('#tbl-remote-items-body').append(datarow);
  });
}

function update_remote_cycles(data) {
  $('#tbl-remote-cycles-body')
    .find('tr')
    .remove()
    .end();
  $.each(data, function(k, v) {
    ...
    datarow.append(
      $('<td />')
        .addClass('vtop')
        .prop('width', 110)
        .html(['stopped', 'running', 'stopping'][v.status])
    );
    datarow.append(
      $('<td />')
        .addClass('vtop')
        .prop('width', 160)
        .html(v.interval.toFixed(4))
    );
    datarow.append(
      $('<td />')
        .addClass('vtop')
        .prop('width', 160)
        .html(parseFloat(v.value).toFixed(4))
    );
    $('#tbl-remote-cycles-body').append(datarow);
  });
}

...
function remove_controller(controller_id) {
  $popup('warning', 'Rmoving controller', 'Do you realy want to remove ' + controller_id + ' controller?', { 
    btn1a: function() {
      eva_sfa_call(
        'remove_controller',
        {i: controller_id},
...
function reload_controller(controller_id) {
  eva_sfa_call(
    ...
    {i: controller_id},
    function() {
      $popup('info', 'Reload requested', controller_id + ' reload requested', {
        ct: 2
      });
      reload_controllers_data();
    },
...
function enable_controller(controller_id) {
  eva_sfa_call(
    ...
    {i: controller_id},
    function() {
      $popup('info', 'Controller enabled', controller_id + ' enabled', {
        ct: 2
      });
      reload_controllers_data();
    },
...
function disable_controller(controller_id) {
  eva_sfa_call(
    ...
    {i: controller_id},
    function() {
      $popup('warning', 'Controller disabled', controller_id + ' disabled', {
        ct: 2
      });
      reload_controllers_data();
    },
...
$(document).ready(function() {
  ...
  /*$.each(tbars, function(k, v) {
    ...
    var btn = $('<input />', {
      ...
      $('.tbar_btn').css('font-weight', 'normal');
  ...
  /*$.each(tbars, function(k, v) {
    var c = $('<div />', {id: 'tbar-' + v.id, class: 'tbar'});
    var tbl = $('<table />', {id: 'tbl-' + v.id, class: 'display tbl'});
    var tr = $('<tr />');
    $.each(v.cols, function(i, x) {
      tr.append($('<th />').html(x));
    });

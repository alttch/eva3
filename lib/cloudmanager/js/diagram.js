...
function main(container, toolbar, sidebar) {
    if (!mxClient.isBrowserSupported()) {
        ...
        startView = function() {
            ...
            try {
                //input
                for (let i in Object.keys(t.input)) {
                    var id = 'input ' + i;
                ...
                body_content[1] = [];
                body_content[2] = [];
                for (let nb in Object.keys(t.body)) {
                    
                    var vert = graph.getModel().cloneCell(global_vertex[t.body[nb].function]);
                    vert.block_name = 'body';
                    vert.id = 'body ' + t.body[nb].id;

                    // var id = 'body ' + t.body[nb].id;
                    // var name = block_name(t.body[nb], 'body');
                    var b_x = 400;
                    var b_y = 100;
                    for(let param in t.body[nb].params) {
                        if(t.body[nb].params[param].type) {
                            if(t.body[nb].params[param].type !== 'IN') {
                                if(g_el_body[t.body[nb].params[param].value] && g_el_body[t.body[nb].params[param].value].geometry.x >= b_x) {
                                    b_x = g_el_body[t.body[nb].params[param].value].geometry.x + 300;
                                }
                            }
                        } else {
                            for(let i in t.body[nb].params[param]) {
                                if(g_el_body[t.body[nb].params[param][i].value] && g_el_body[t.body[nb].params[param][i].value].geometry.x >= b_x) {
                                    b_x = g_el_body[t.body[nb].params[param][i].value].geometry.x + 300;
                                }
                            }
                        }
                    }
                ...
                for (var b in g_el_body) {
                    ...
                    if (x) {
                        if (t.body[b].params.item_id.type === 'var_in') {
                            ...
                            let inp = false;
                            for (let ch in g_el_body[b].children) {
                                if (g_el_body[b].children[ch].type === 'var_in') {
                                    inp = g_el_body[b].children[ch];
                                    break;
                                }
                            }
                            if (!inp) {
                                inp = graph.insertVertex(g_el_body[b], null, block_name(Object.keys(t.body[b].params)[0]), 10, 90, 50, 30, 'body_child');
                                inp['type'] = 'var_in';
                            }
                            ...
                            let inp = false;
                            for (let ch in g_el_body[b].children) {
                                if (g_el_body[b].children[ch].type === 'const' || g_el_body[b].children[ch].type === 'var_in') {
                                    inp = g_el_body[b].children[ch];
                                    break;
                                }
                            }
                            if (!inp) {
                                inp = graph.insertVertex(g_el_body[b], null, block_name(Object.keys(t.body[b].params)), 10, 90, 50, 30, 'body_child');
                            }
                            inp['type'] = 'const';
                    ...
                    if (isKeyExist(t.body[b].params, 'args') || isKeyExist(t.body[b].params, 'IN')) {
                        ...
                        let inp = false;
                        for (let ch in g_el_body[b].children) {
                            if (g_el_body[b].children[ch].type === 'args') {
                                inp = g_el_body[b].children[ch];
                                break;
                            }
                        }
                        if (!inp) {
                            inp = graph.insertVertex(g_el_body[b], null, block_name('args'), 10, 130, 50, 30, 'body_child');
                            inp['type'] = 'args';
                        }
                        if (inp_value) {
                            ...
                            for (let a in args_values) {
                                ...
                                for (let b in g_el_body) {
                                    if (b_id === +g_el_body[b].id.split(' ')[1]) {
                                        let out = false;
                                        for (let ch in g_el_body[b].children) {
                                            if (g_el_body[b].children[ch].type === 'out') {
                                                out = g_el_body[b].children[ch];
                                                break;
                                            }
                                        }
                                        if (!out) {
                        ...
                        if (isKeyExist(t.body[b].params, 'IN')) {
                            if (t.body[b].params.IN.type === 'const') {
                                let inp = false;
                                for (let ch in g_el_body[b].children) {
                                    if (['in_const','in_block_out'].includes(g_el_body[b].children[ch].type)) {
                                        inp = g_el_body[b].children[ch];
                                        break;
                                    }
                                }
                                if (!inp) {
                                    inp = graph.insertVertex(g_el_body[b], null, block_name('IN'), 10, 90, 50, 30, 'body_child');
                                }
                                inp['type'] = 'in_const';
                                let x = g_el_body[b].geometry.x;
                                let y = g_el_body[b].geometry.y;
                                ...
                                let inp = false;
                                for (let ch in g_el_body[b].children) {
                                    if (g_el_body[b].children[ch].type === 'in_var_in') {
                                        inp = g_el_body[b].children[ch];
                                        break;
                                    }
                                }
                        ...
                        for(let param in t.body[b].params) {
                            let inp = false;
                            for (let ch in g_el_body[b].children) {
                                if (['in_const','in_block_out'].includes(g_el_body[b].children[ch].type) && g_el_body[b].children[ch].name === param) {
                                    inp = g_el_body[b].children[ch];
                                    break;
                                }
                            }
                            if (!inp) {
                                inp = graph.insertVertex(g_el_body[b], null, block_name(param), 10, 90 + 40 * Object.keys(t.body[b].params).indexOf(param), 50, 30, 'body_child');
                                inp['type'] = 'in_block_out';
                            }
                            for (var i in g_el_body) {
                                if (t.body[b].params[param].value === +g_el_body[i].id.split(' ')[1]) {
                                    let out = false;
                                    for (let ch in g_el_body[i].children) {
                                        if (g_el_body[i].children[ch].type === 'out' && 
                                                t.body[b].params[param].type.split('_').slice(2).join(' ') === g_el_body[i].children[ch].name) {
                                            out = g_el_body[i].children[ch];
                                            break;
                                        }
                                    }
                                    if (!out) {
                                        out = graph.insertVertex(g_el_body[i], null, block_name('out'), 90, 90, 50, 30, 'body_child');
                                    }
                                    out['type'] = 'out';
                                    graph.insertEdge(parent, null, '', inp, out, 'body_ch');
                                }
                ...
                for (var o = 0; o < t.output.length; o++) {
                    ...
                    g_el_out[o] = graph.insertVertex(parent, id, name, +Object.keys(b_coord).slice(-1)[0] + 300, (o === 0 ? y : y + o * 200), 140, 70, "output");
                    ...
                    for (var elem in g_el_body) {
                        if (out === Number(g_el_body[elem].id.split(' ').pop())) {
                            let out = null;
                            for(let ch_o in g_el_body[elem].children) {
                                if(g_el_body[elem].children[ch_o].type == 'out' && (g_el_body[elem].children[ch_o].name == 'out' ||
                                        g_el_body[elem].children[ch_o].name === t.output[o].name)) {
                                    out = g_el_body[elem].children[ch_o];
                                    break;
                                }
                            }
                            if(out === null) {
                                out = graph.insertVertex(g_el_body[elem], null, block_name('out'), 90, 130, 50, 30, 'body_child');
                                out['type'] = 'out';
                            }

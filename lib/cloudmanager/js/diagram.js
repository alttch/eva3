function main(container, toolbar, sidebar) {
    if (!mxClient.isBrowserSupported()) {
        ...
        document.getElementById('title').innerText = t.function.toUpperCase();
        ...
        //add style
        mxClient.link('stylesheet', '/static/css/modal.css');
        ...
        mxConnectionHandler.prototype.connect = function (source, target, evt, dropTarget, style) {
            ...
            if (source.type) {
                ...
                if (target.type) {
                    ...
                    if (source.type === 'out' && target.type === 'out' || t.length > 0 ||
                        source.parent.id === target.parent.id || source.type === target.type) {
                        return false;
                    }
                    ...
                    if (('var_in, const, in_const'.includes(source.type) || source.type === 'in_block_out') && target.type !== 'args') {
        ...
        try {
            ...
            for (var i = 0; i < t.input.length; i++) {
                ...
                g_el_in[i] = graph.insertVertex(parent, id, name, x, (i === 0 ? y : y + i * 400), 140, 70, 'input');
            ...
            for (var o = 0; o < t.output.length; o++) {
                ...
                g_el_out[o] = graph.insertVertex(parent, id, name, 1300, (o === 0 ? y : y + o * 400), 140, 70, "output");
...
function showEdit(graph, cell) {
    ...
    wnd = showModalWindow('EDIT', form.table, 300, 250);
}

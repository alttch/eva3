...
function redraw_lvar_state(i) {
  ...
  $('[id="lval_expires_' + i + '"]').html(format_expire_time(items[i]));
...
function update_lvar_timers() {
  $.each(items, function(k, i) {
    if (i.status == 1 && i.value != 'null') {
      if (i.expires > 0) {
        $('[id="lval_expires_' + k + '"]').html(format_expire_time(i));
      }
    } else {
...
function update_rule_groups(groups) {
  ...
  $('#s_rule_group').find('option').remove().end();
  $('#s_rule_group').show();
...
function set_lvar_state(uid) {
  ...
  $.getJSON(
    '/lm-api/set?k=' + apikey + '&i=' + uid + '&s=' + s + '&v=' + v,
    function(data) {
      if (data && data.result == 'OK') {
        items[uid].status = s;
        items[uid].value = v;
  ...
  $.getJSON(
    ...
    function(data) {
      ...
      $.each(data.sort(dynamic_sort('id')), function(_k) {
        var uid = data[_k]['id'];
        var val = data[_k];
        _macro = $('<div />', {class: 'col-md-9 col-sm-8 col-xs-1btn col-xs-12'});
        _macro_buttons = $('<div />', {
...
function edit_rule_dialog(i) {
  ...
  if (i === undefined) {
    ...
    if (dm_rules[i].macro_args != null) _macro_args = dm_rules[i].macro_args;

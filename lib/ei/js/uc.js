...
function btn_action(uid, s) {
  ...
  $.getJSON(
    '/uc-api/action?k=' + apikey + '&i=' + uid + '&s=' + s + '&w=120',
    function(data) {
...
function kill_unit(i) {
  $.getJSON('/uc-api/kill?k=' + apikey + '&i=' + i, function(data) {
  ...
  $.getJSON(
    ...
    function(data) {
      $.each(data, function(_k) {
        ...
        if (items[uid].status != val.status || items[uid].value != val.value) {
          items[uid] = val;
...
function update_unit_state(uid) {
  ...
  $.getJSON(
    '/uc-api/update?k=' + apikey + '&i=' + uid + '&s=' + s + '&v=' + v,
    function(data) {
      if (data && data.result == 'OK') {
        items[uid].status = s;
        items[uid].value = v;
  ...
  $.getJSON(
    ...
    function(data) {
      ...
      $.each(Object(data).sort(dynamic_sort('id')), function(_k) {
        var uid = data[_k]['id'];
        var val = data[_k];
        _unit = $('<div />', {class: 'col-md-5 col-sm-8 col-xs-2btn col-xs-12'});
        _unit_actions = $('<div />', {class: 'col-md-3 col-sm-2 col-xs-btn col-xs-12'});
        items[uid] = val;
        ...
        _unit_buttons = $('<div />', {class: 'col-md-4 col-sm-2 col-xs-btn col-xs-12'});
        //if (master) {
        //$('<button />', { 'class': 'st0',
...
function set_sensor_status(i, st) {
  $.getJSON(
    '/uc-api/update?k=' + apikey + '&i=' + i + '&s=' + st + '&force_virtual=1',
    function(data) {
      if (data && data.result == 'OK') {
        items[i].status = st;

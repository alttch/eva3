...
function load_log_entries(r, scroll) {
    ...
    $.getJSON('/sys-api/log_get?l=' + log_level + '&n=' + max_log_records +
        '&k=' + apikey,
        function( data ) {
            ...
            $.each( lr2p, function (i, v) { t.append(v) } )
            if (scroll || log_autoscroll) t.scrollTop(t.prop('scrollHeight'));
            if ((!ws_mode && log_first_load) || r) {
                setTimeout(
                    function() { load_log_entries(true, false) }, lrInterval)
            }
            if (log_first_load) { log_first_load = false }
        }).error(
        function(data) {
            if ((!ws_mode && log_first_load) || r) {
                setTimeout(
...
function log_record(l) {
    return '<div class="logentry logentry_color_' + l.l + '">' +
        time_converter(l.t) + ' ' + l.h + ' ' + l.p + ' ' +
        log_level_name[l.l] + ' ' + l.mod + ' ' + l.th + ': ' +
        l.msg + '</div>'
}
...
function append_log_entry(l) {
    if (log_loaded) {
        ...
        while ($('.logentry').length > max_log_records) {
            $('#logr').find('.logentry').first().remove();
        }
    } else {
        lr2p.push(l)
    }
...
function ask_debug_mode() {
    if (master && !debug_mode && log_level<=10) {
        popup('confirm', 'Enable DEBUG',
            'Server is not in the DEBUG MODE<br />Enable DEBUG?',
            'YES', 'NO', 'set_debug_mode(true)', '')
    }
}

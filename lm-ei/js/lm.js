...
var dm_rule_for_props = [ 'status', 'value' ] 
...
var toolbars = [ 'blank', 'lvars', 'macros', 'rules', 'log' ]
var boards = [ 'blank', 'keyform', 'lvars', 'macros', 'rules', 'log' ]
...
function reset_lvar(i) {
    $.getJSON( '/lm-api/reset?k=' + apikey + '&i=' + i,
            function (data) {
                if (data && data.result == 'OK') {
                    ...
                    popup('error', 'ERROR',
                            'Reset command failed for ' + i)
                }
            }).fail(function(){popup('error', 'ERROR',
                            'Server error. Reset command failed for ' + i)})
}
...
function load_lvar_state() {
    ...
    $.getJSON( '/lm-api/state?k=' + apikey + '&g=' +
        $('#s_lvar_group').val(),
        function( data ) { 
            $.each( data, function( _k ) { 
                ...
                if (items[uid].status != val.status ||
                        items[uid].value != val.value ||
                        items[uid].set_time != val.set_time ||
                        items[uid].expires != val.expires ) {
                    items[uid] = val 
...
function load_lvar_data() {
    ...
    $.getJSON( '/lm-api/groups?k=' + apikey + '&p=LV',
        function( data ) {
            $('#s_lvar_group').find('option').remove().end()
            var ng = false
            var gf = false
            ...
            if (gf || ng) {
                ...
                $('#b_lvars').html(
                        '<div class="notavail">No LVars available</div>')
            }
        })
}
...
function load_macros_data() {
    ...
    $.getJSON( '/lm-api/groups_macro?k=' + apikey,
        function( data ) {
            $('#s_macro_group').find('option').remove().end()
            var ng = false
            var gf = false
            $.each( data, function (uid, val) {
                if (val != 'nogroup') {
            ...
            if (gf || ng) {
                ...
                $('#b_macros').html(
                        '<div class="notavail">No macros available</div>')
            }
      })
}


function set_lvar_state(uid) {
    var s = $('#lvar_status').val()
    var v = $('#lvar_value').val()
    $.getJSON( '/lm-api/set?k=' + apikey + '&i=' + uid +
                '&s=' + s + '&v=' + v,
            function (data) {
                if (data && data.result == 'OK') {
                    items[uid].status = s
                    ...
                    popup('error', 'ERROR',
                        'LVar not changed. Result: ' +
                            data.result)
                }
            }).fail(function(){popup('error', 'ERROR',
                            'Server error. LVar not changed')})
}
...
function select_lvar_state(uid) {
    ...
    $.each(lvar_state_labels, function (_k, v) {
        ...
        if (_k == items[uid].status) html += ' selected'
        html += '>' + v + '</option>'
        })
    var value = items[uid].value
    if (value == 'null') value = ''
    html += '</select></div></div>' + 
            '<div class="form-group">' +
                '<label class="col-xs-6 control-label"' +
                ...
                '<input class="form-control" type="text" size="5"' +
                    ...
                    '" /></div></div></form>'
    popup('confirm', 'SET STATE OF ' + uid, html, 'SET', 'CANCEL',
            'set_lvar_state("' + uid + '")')
}
...
function load_lvars() {
  $.getJSON( '/lm-api/state?k=' + apikey + '&full=1&g=' +
        $('#s_lvar_group').val(),
        function( data ) {
            var bg = 1
            $('#b_lvars').html('')
            $.each( Object(data).sort(),
                function( _k ) {
                    ...
                    $('<span />', { 'class': 'lval',
                        html: _e_label }).appendTo(_lvar_expires)
                    $('<span />', { id: 'lval_expires_' + uid,
                        'class': 'lval',
                        html: format_expire_time(val.set_time, val.expires)
                        }).appendTo(_lvar_expires)
                    $('<span />', { 'class': 'lval',
                        html: 'V='}).appendTo(_lvar_state)
                    ...
                    var iname = uid
                    $('<div />', { 'class': 'device lvar_s' + val.status,
                        id: 'lname_' + uid, html: iname }).appendTo(_lvar)
                    $('<div />', { 'class': 'device-descr',
                        html: val['description'] }).appendTo(_lvar)
                    _lvar.appendTo(_r)
...
function prepare_macro_run(i) {
    ...
    popup('confirm', 'Run macro "' + i + '"', html, 'RUN', 'CANCEL',
            'run_macro("' + i + '")')
...
function enable_disable_macro(i, e) {
    $.getJSON( '/lm-api/set_macro_prop?k=' + apikey + '&i=' + i +
            ...
            function (data) {
                if (data && data.result == 'OK') {
                    ...
                    btn.attr('onclick', 'enable_disable_macro("' + i + '", ' +
                                    (e==0?1:0) + ')')
                    btn.html(e==0?'DISABLED':'ENABLED')
                } else {
                    popup('error', 'ERROR',
                        'Parameter not changed. Result: ' +
                            data.result)
                }
            }).fail(function(){popup('error', 'ERROR',
                        'Server error. Parameter not changed')})
}


function enable_disable_rule(i, e) {
    $.getJSON( '/lm-api/set_rule_prop?k=' + apikey + '&i=' + i +
            '&p=enabled&v=' + e,
            function (data) {
                if (data && data.result == 'OK') {
                    ...
                    btn.attr('onclick', 'enable_disable_rule("' + i + '", ' +
                                    (e==0?1:0) + ')')
                    btn.html(e==0?'DISABLED':'ENABLED')
                } else {
                    popup('error', 'ERROR',
...
function load_macros() {
  $.getJSON( '/lm-api/list_macros?k=' + apikey + '&g=' +
        $('#s_macro_group').val(),
        function( data ) {
            var bg = 1
            $('#b_macros').html('')
                ...
                function( _k ) {
                    ...
                    if (master) {
                        $('<button />', {
                            ...
                            'class': 'st0' +
                                (val.action_enabled==0?'':' active'),
                            'onclick': 'enable_disable_macro("' + uid + '", ' +
                                    (val.action_enabled==0?1:0) + ')',
                            html:
                                (val.action_enabled==0?'DISABLED':'ENABLED') }
                    ...
                    var _r = $('<div />', { 'class': 'row row-device bg' + bg})
                    $('<div />', { id: 'macro_name_' + uid,
                        'class': 'device',
                        html: uid }).appendTo(_macro)
                    $('<div />', { 'class': 'device-descr',
                        html: val['description'] }).appendTo(_macro)
                    _macro.appendTo(_r)
...
function dm_rule_for_expire(rule) {
    if (rule.for_prop == 'status' &&
            ...
            rule.in_range_max == -1 &&
            rule.in_range_max_eq) return true
    return false
}
...
function del_rule(i) {
    $.getJSON( '/lm-api/destroy_rule?k=' + apikey + '&i=' + i,
            function (data) {
                if (data && data.result == 'OK') {
                    $('#rule_' + i).remove()
                } else {
                    popup('error', 'ERROR',
                        'Unable to delete rule. Result: ' +
                            data.result)
                }
            }).fail(function(){
                popup('error', 'ERROR',
                    'Server error. Unable to delete rule')})
}


function ask_del_rule(i) {
    popup('warning', 'DELETE RULE',
            'Rule ' + i + ' will be deleted. Please confirm',
            'DELETE', 'CANCEL', 'del_rule("' + i + '")')
}
...
function rule_from_edit_dialog() {
    ...
    rule.chillout_time = $('#rule_chillout').val().replace(',', '.')
    ...
    if (cond == 'expire') {
        ...
        var m = $('#rule_in_range_min').val().replace(',', '.')
        if (isNumeric(m)) {
            rule.in_range_min = m
            rule.in_range_max = m
        ...
        rule.in_range_min = $('#rule_in_range_min_r').val().replace(',', '.')
        rule.in_range_max = $('#rule_in_range_max_r').val().replace(',', '.')
...
function validate_rule_dialog() {
    ...
    if (rule._cond == 'range') {
        if (rule.in_range_min != '' && (!isNumeric(rule.in_range_min) ||
               (isNumeric(rule.in_range_min) && isNumeric(rule.in_range_max) &&
        ...
        if (rule.in_range_max != '' && !isNumeric(rule.in_range_max) ||
               (isNumeric(rule.in_range_min) && isNumeric(rule.in_range_max) &&
                    (Number(rule.in_range_min) > Number(rule.in_range_max)))) {
...
function edit_rule_dialog(i) {
    ...
    if (i === undefined) {
        ...
        if (dm_rules[i].macro != null)
            _macro = dm_rules[i].macro
        if (dm_rules[i].macro_args != null)
            _macro_args = dm_rules[i].macro_args
    ...
    html += '<option value="1"' + (_in_range_min_eq?' selected':'') +
                '>&lt;=</option>'
    html += '</select>'
    html += '</div>'
    ...
    html += '<option value="0">&lt;</option>'
    html += '<option value="1"' + (_in_range_max_eq?' selected':'') +
                '>&lt;=</option>'
    html += '</select>'
    html += '</div>'
    html += '<div class="col-xs-4 col-sm-2" id="d_rule_cond_range_max">' +
                        '<input class="form-control" type="text" size="5"' +
                        'id="rule_in_range_max_r" value="' + _in_range_max +
                    '"></div></div>'
    // row 6
    html += '<div class="form-group">' +
                    '<label class="col-xs-3 control-label"' +
    ...
    html += '' +
                ...
                '<input class="form-control" type="text" size="5"' +
                    ...
                    '" /></div></div>'
    html += '</form>'
    popup('!confirm', _title, html, 'OK', 'CANCEL',
            'set_rule_props_ae("' + i + '")', null, validate_rule_dialog)
    ...
    $.post('/lm-api/' + f, d,
            function (data) {
                if (data && data.result == 'OK') {
                    ...
                    popup('error', 'ERROR',
                            'Unable to process rule')
                }
                load_rules()
            }, dataType = 'json').fail(function(){
                        popup('error', 'ERROR',
                                'Server error. Unable to process rule')
                        load_rules()
                        })
}
...
function load_rules() {
  $.getJSON( '/lm-api/list_rules?k=' + apikey,
        function( data ) {
            var bg = 1
            show_toolbar('rules')
            $('#b_rules').html('')
            $.each( data,
                function( _k ) {
                    ...
                    $('<button />', {
                        ...
                        'class': 'st0' +
                            (val.enabled==0?'':' active'),
                        'onclick': 'enable_disable_rule("' + uid + '", ' +
                                (val.enabled==0?1:0) + ')',
                        html:
                            (val.enabled==0?'DISABLED':'ENABLED') }
                    ...
                    var _r = $('<div />', { id: 'rule_' + uid,
                        'class': 'row row-device bg' + bg})
                    $('<div />', { id: 'rule_priority_' + uid,
                        'class': 'rule-priority',
                        html: val.priority }).appendTo(_rule)
                    $('<div />', { id: 'rule_id_' + uid,
                        'class': 'rule-id',
                        html: uid }).appendTo(_rule)
                    $('<div />', { 'class': 'rule-desc',
                        html: val['description'] }).appendTo(_rule)
                    $('<div />', { 'class': 'device-desc',
                        html:
                            rn2a(val['for_item_type']) + ':' +
                            rn2a(val['for_item_group']) + '/' +
                    ...
                    $('<div />', { 'class': 'rule-condition',
                        html: val['condition'] + exp }).appendTo(_rule_info)
                    $('<div />', { 'class': 'rule-info',
                        html: 'Initial: <b>' +
                            rs_for_init(val['for_initial']) + 
                                    '</b>. Break: <b>' +
                            (val['break_after_exec']?'Y':'N') + '</b>'
                                     }).appendTo(_rule_info)
                    $('<div />', { 'class': 'rule-info',
                        html: 'Chillout: <b>' +
                            val['chillout_time'] +
                            '</b> sec' }).appendTo(_rule_info)
                    if (val['macro'] != null) {
                        var m = '<b>' + val['macro'] + '</b>'
                        ...
                        $('<div />', { 'class': 'rule-info',
                            html: 'Macro: "' + m + '"' }).appendTo(_rule_info)
                        }
                    _rule.appendTo(_r)
...
function process_ws(evt) {
    ...
    if (data.s == 'log') {
        $.each (data.d, function ( i, l ) { append_log_entry(log_record(l)) })
        if (log_autoscroll) {
...
function start_ws(reload) {
    ...
    if (loc.protocol === 'https:') { uri = 'wss:'; } else { uri = 'ws:'; }
    uri += '//' + loc.host;
    ws = new WebSocket(uri + '/ws?k=' + apikey);
    ws.onmessage = function(evt) { process_ws(evt); };
    ws.onclose = function() {
        $('#i_connection').html('<b><span style="color: red">' +
            'WS connecting' + '</span></b>')
        log_first_load = true
        lvars_loaded = false
        log_loaded = false
        setTimeout(function() { start_ws(true) }, 3000);
      };
    ws.addEventListener('open',
        function (event) {
            $('#i_connection').html(
                    '<b><span style="color: green">instant</span></b>')
        ws.send(JSON.stringify({'s': 'state' }))
        if (log_subscribed) { ws.send(JSON.stringify({'s': 'log' })) }
        if (reload) {
            log_first_load = true
...
function init_dashboard(reload) {
    ...
    if (ws_mode) {
        ...
        $('#i_connection').html('<b><span style="color: orange">' +
                jrInterval/1000 + ' sec' + '</span></b>')
        if (!reload) {
...
function set_debug_mode(mode) {
    $.getJSON('/sys-api/set_debug?k=' + apikey + '&debug=' + (mode?'1':'0'),
        function(data) {
            if (data && data.result == 'OK') {
        ...
        }).fail(function(){popup('error', 'ERROR', 'Server error')})
}


function save() {
    $.getJSON('/sys-api/save?k=' + apikey,
        function(data) {
            if (data && data.result == 'OK') {
            ...
            function(data) {
                ...
                popup('error','ERROR',
                    'Unable to update system data<br />UNKNOWN ERROR')
            })
}
...
function show_debug_info(d) {
    if (debug_mode) {
        $('#i_debug').css('color', 'orange').css(
                'font-weight', 'bold').html('Debug mode: ON')
        if (master) {
            $('#i_debug').css('cursor', 'pointer').attr(
                    'onclick', 'set_debug_mode(false)')
        }
    } else {
        $('#i_debug').css('color', '#4D4D4D').css(
                'font-weight', 'normal').html('Debug mode: OFF')
        if (master) {
            $('#i_debug').css('cursor', 'pointer').attr(
                    'onclick','set_debug_mode(true)')
        }
    }
}
...
function disable_sys_func() {
    $('#tbtn_log').addClass('disabled').attr('onclick', null)
    $('#tbtn_save').addClass('disabled').attr('onclick', null)
}


function enable_sys_func() {
    $('#tbtn_log').removeClass('disabled').attr(
            'onclick','safe_close_tool_menu();show_log()')
    $('#tbtn_save').removeClass('disabled').attr(
            'onclick','safe_close_tool_menu();save()')
}


function disable_master_func() {
    $('#tbtn_rules').addClass('disabled').attr('onclick', null)
}


function enable_master_func() {
    $('#tbtn_rules').removeClass('disabled').attr(
            'onclick','safe_close_tool_menu();show_rules()')
}
...
function login(k, remember_k, initial, reload) {
    ...
    $.getJSON('/lm-api/test?k=' + k,
        function( data ) {
            ...
            if (initial || reload) {
                if (data.acl.master || (('sysfunc' in data.acl) &&
                            data.acl.sysfunc)) {
                    enable_sys_func()
                } else {
                    disable_sys_func()
...
function invalid_api_key() {
    popup('error', 'ACCESS DENIED', 'INVALID API KEY',
                'OK', '', 'show_login_form()', '');
}

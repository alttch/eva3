...
function popup(pclass, title, msg, btn1, btn2, btn1a, btn2a, va) {
    ...
    $('#popup_btn_2').html('')
    if (va === undefined) {
        _popup_validated = function () { return true; }
    } else {
        _popup_validated = va
    }
    var btn1text = 'OK'
    ...
    var btn1 = $('<div />',
            { 'class': 'popup_btn popup_btn_' + _pclass, html: btn1text })
    if (btn1a) {
        btn1.attr('onclick', 'if (_popup_validated()) {' +
            'close_popup();' + btn1a + ';}')
    } else {
    ...
    if (btn2) {
        var btn2 = $('<div />',
            { 'class': 'popup_btn popup_btn_' + _pclass, html: btn2 })
        if (btn2a) {

 ...
 *
 * @param obj_key - object key in format obj:full_id, i.e. sensor:env/temp1
 * @param cb - function to be called
 *
 * if state is already loaded, function will be called immediately
 */
function eva_sfa_register_update_state(obj_key, cb) {
    eva_sfa_update_state_functions[obj_key] = cb
    var state = eva_sfa_state(obj_key)
...
function eva_sfa_register_rule(rule_id, cb) {
    eva_sfa_rules_to_monitor.push(
            { 'rule_id': rule_id, 'cb': cb }
            )
 ...
 */
function eva_sfa_state(obj_key) {
    if (obj_key in eva_sfa_states) {
        return eva_sfa_states[obj_key]
    } else {
        return undefined
    }
}
 ...
 */
function eva_sfa_status(obj_key) {
    var state = eva_sfa_state(obj_key)
 ...
 */
function eva_sfa_value(obj_key) {
    var state = eva_sfa_state(obj_key)
    if (state === undefined || state === null) return undefined
 ...
 */
function eva_sfa_run(macro_id, args, wait, priority, uuid,
            cb_success, cb_error) {
    var q = ''
    ...
    $.getJSON( '/sfa-api/run?' + q,
        function( data ) {
            if (cb_success !== undefined && cb_success !== null)
                cb_success(data)
            }
    ).error(
        function(data) {
...
function eva_sfa_action(
    ...
    $.getJSON( '/sfa-api/action?' + q,
        ...
        function(data) {
            if (cb_error !== undefined && cb_error !== null)
                cb_error(data)
        }
    )
}
...
function eva_sfa_set_rule_prop(
        rule_id, prop, value, save, cb_success, cb_error) {
    var q = ''
    if (eva_sfa_apikey !== null && eva_sfa_apikey != '') {
        q += 'k=' + eva_sfa_apikey
    }
...
function eva_sfa_after_login(data) {
    ...
    if (!eva_sfa_ws_mode) {
        ...
        if (eva_sfa_force_reload_interval) {
            eva_sfa_ajax_reload = setInterval(
                function(){eva_sfa_load_initial_states(true)},
...
function eva_sfa_process_state(state) {
    var obj_key = state.type + ':' + state.group + '/' + state.id
    eva_sfa_states[obj_key ] = state
    if (obj_key in eva_sfa_update_state_functions) {
        var f = eva_sfa_update_state_functions[obj_key]
...
function eva_sfa_start_ws() {
    ...
    eva_sfa_ws.onmessage = function(evt) { eva_sfa_process_ws(evt); };
    eva_sfa_ws.addEventListener('open',
        function (event) {
...
function eva_sfa_heartbeat(on_login, data) {
    ...
    if (eva_sfa_ws_mode) {
        if (eva_sfa_last_ping !== null) {
            if (eva_sfa_last_pong === null ||
                    (eva_sfa_last_ping - eva_sfa_last_pong) >
    ...
    $.getJSON('/sfa-api/test' + q,
        function (data) {
            eva_sfa_server_info = data
            if (on_login !== undefined && on_login) {
                if (eva_sfa_cb_login_success !== null)
                    eva_sfa_cb_login_success(data)
            }
        }).error(
            function(data) {
                if (eva_sfa_heartbeat_error !== null) {
...
function eva_sfa_load_rule_props(rule_id, cb) {
    ...
    if (eva_sfa_apikey !== null && eva_sfa_apikey != '')
        q += 'k=' + eva_sfa_apikey
    q+= '&i=' + rule_id
    $.post('/sfa-api/list_rule_props', q,
            function (data) {

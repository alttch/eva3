...
function ask_kill(uid) {
    popup('confirm', 'KILL ' + uid,
            'Terminate ALL running and queued actions?',
            'YES', 'NO', 'kill_unit("' + uid + '")', '')
}

function kill_unit(i) {
    $.getJSON( '/uc-api/kill?k=' + apikey + '&i=' + i,
            function (data) {
                if (data && data.result == 'OK') {
                    var msg = 'All actions of ' + i + ' killed'
                    var title = 'KILLED ' + i
                    var popt = 'info'
                    ...
                    popup(popt, title, msg + '. Action queue cleared')
                } else {
                    popup('error', 'ERROR',
                        'Kill command failed for ' + i + '. Result: ' +
                            data.result)
                }
...
function enable_disable_actions(i, e) {
    $.getJSON( '/uc-api/' + e + '_actions?k=' + apikey + '&i=' + i,
            function (data) {
                if (data && data.result == 'OK') {
...
function redraw_unit(uid) {
    ...
    if (items[uid].action_enabled) {
        eb.html('ENABLED')
        eb.addClass('active')
        eb.attr('onclick', 'enable_disable_actions("' + uid +'", "disable")')
        $('button[id^=btn_' + uid + '_]').removeAttr('disabled')
    } else {
        eb.html('DISABLED')
        eb.removeClass('active')
        eb.attr('onclick', 'enable_disable_actions("' + uid +'", "enable")')
        $('button[id^=btn_' + uid + '_]').attr('disabled', 'disabled')
    }
}
...
function load_unit_state() {
    ...
    $.getJSON( '/uc-api/state?k=' + apikey + '&g=' +
        $('#s_unit_group').val() + '&p=U',
        function( data ) {
            $.each( data, function( _k ) {
                ...
                if (
                        items[uid].status != val.status ||
                        items[uid].action_enabled != val.action_enabled
                        ) {
                    items[uid] = val
                    redraw_unit(uid)
                }
...
function load_unit_data() {
    ...
    $.getJSON( '/uc-api/groups?k=' + apikey + '&p=U',
        function( data ) {
            $('#s_unit_group').find('option').remove().end()
            var ng = false
            var gf = false
            ...
            if (gf || ng) {
                ...
                $('#b_units').html(
                        '<div class="notavail">No units available</div>')
            }
        })
}
...
function load_sensor_data() {
    ...
    $.getJSON( '/uc-api/groups?k=' + apikey + '&p=S',
        function( data ) {
            $('#s_sensor_group').find('option').remove().end()
            var ng = false
            var gf = false
            $.each( data, function (uid, val) {
                if (val != 'nogroup') {
            ...
            if (gf || ng) {
                ...
                $('#b_sensors').html(
                        '<div class="notavail">No sensors available</div>')
            }
      })
}


function update_unit_state(uid) {
    var s = $('#unit_status').val()
    var v = $('#unit_value').val()
    $.getJSON( '/uc-api/update?k=' + apikey + '&i=' + uid +
                '&s=' + s + '&v=' + v,
            function (data) {
                if (data && data.result == 'OK') {
                    items[uid].status = s
...
function select_unit_state(uid) {
...
function load_units() {
  $.getJSON( '/uc-api/state?k=' + apikey + '&full=1&g=' +
        $('#s_unit_group').val() + '&p=U',
        function( data ) {
            var bg = 1
            $('#b_units').html('')
            $.each( Object(data).sort(dynamic_sort('id')),
                function( _k ) {
                    ...
                    if (val.action_enabled) {
                        $('<button />', { 'class': 'st0 active',
                            id: 'btn_enable_' + uid,
                            html: 'ENABLED' }).attr(
                                'onclick','enable_disable_actions("' + uid +
                                    '", "disable")').appendTo(_unit_buttons)
                    } else {
                        $('<button />', { 'class': 'st0',
                            id: 'btn_enable_' + uid,
                            html: 'DISABLED' }).attr(
                                'onclick','enable_disable_actions("' + uid + 
                                    '", "enable")').appendTo(_unit_buttons)
                    }
                    if (!val.virtual) {
                        $('<button />', { 'class': 'st0',
                            html: 'SET' }).attr(
                                'onclick','select_unit_state("' +
                                    uid + '")').appendTo(_unit_buttons)
                    }
                    $('<button />', { 'class': 'st0',
                        html: 'KILL' }).attr(
                            'onclick','ask_kill("' + uid + '")'
                    ...
                    $('<div />', { 'class': 'device' +
                            (val.status == -1?' device-error':''),
                        id: 'uname_' + uid, html: iname }).appendTo(_unit)
                    $('<div />', { 'class': 'device-descr',
                        html: val['description'] }).appendTo(_unit)
                    _unit.appendTo(_r)
                    _unit_actions.appendTo(_r)
                    _unit_buttons.appendTo(_r)
                    _r.appendTo('#b_units')
                    if (!val.action_enabled) {
                        $('button[id^=btn_' + uid + '_]').attr(
                                'disabled', 'disabled')
                    }
...
function set_sensor_status(i, st) {
    $.getJSON( '/uc-api/update?k=' + apikey + '&i=' + i +
                '&s=' + st + '&force_virtual=1',
            function (data) {
                if (data && data.result == 'OK') {
                    items[i].status = st
                    ...
                    popup('error', 'ERROR',
                        'Sensor status not changed. Result: ' +
                            data.result)
                }
            }).fail(function() {
        ...
        function( data ) {
            ...
            $.each( Object(data).sort(dynamic_sort('id')),
                function( _k ) {
                    ...
                    //}
                    $('<button />', {
                        id: 'btn_s_enable_' + uid,
                        'class': 'st0' + (val.status==0?'':' active'),
                        'onclick': 'set_sensor_status("' + uid + '", ' +
                                (val.status==0?1:0) + ')',
                    ...
                    $('<div />', { id: 'sname_' + uid,
                        ...
                        html: iname }).appendTo(_sensor)
                    $('<div />', { 'class': 'device-descr',
                        html: val['description'] }).appendTo(_sensor)
                    _sensor.appendTo(_r)

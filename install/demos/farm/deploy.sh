...
APIS="10.27.11.101:8812 10.27.11.102:8812 10.27.11.111 10.27.11.199:8828"
...
while [ 1 ]; do
    echo -n .
    for a in ${APIS}; do
        curl -m3 -sd "k=demo123" http://${a}/sys-api/test|grep '"result": "OK"' > /dev/null 2>&1
        if [ $? -ne 0 ]; then
            sleep 1
            continue
        fi
...
echo
echo Open http://localhost:8828/ in browser to enter UI, or execute
echo
